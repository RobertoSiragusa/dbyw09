				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: function enter", __func__);
	codec_data = snd_soc_component_get_drvdata(component);
	if (!codec_data) {
		dev_err(component->dev, "%s: codec_data is NULL\n",
			__func__);
	...
	dev_info(component->dev, "%s: DP ctl id %d Stream id %d\n", __func__,
		codec_data->ctl[dai_id], codec_data->stream[dai_id]);
	...
	pr_info("%s error checking for cable status and rc", __func__);
	...
	pr_info("%s checking cable status", __func__);
	...
	pr_info("%s cable state %d", __func__, cable_state);
	...
	pr_info("%s fetching display type", __func__);
	disp_type = codec_data->ext_disp_ops.get_intf_id(
						codec_data->ext_disp_core_pdev);
	pr_info("%s display type %d", __func__, disp_type);
	mutex_unlock(&codec_data->dp_ops_lock);
	if (disp_type >= 0) {
		switch (disp_type) {
	...
	pr_info("%s::%d function exit", __func__, __LINE__);
	return rc;
