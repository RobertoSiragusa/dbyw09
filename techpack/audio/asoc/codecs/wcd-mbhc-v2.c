		      ...
		      bool impedance_det_en)
{
	...
	const char *is_fac_mode_remove = "qcom,is_factory_mode_remove";
	...
	ret = of_property_read_u32(card->dev->of_node, is_fac_mode_remove, &fac_rmv);
	if (ret) {
		dev_warn(card->dev,
			"%s: missing %s in dt node\n", __func__, is_fac_mode_remove);
		fac_rmv = 0;
	}
	...
	if (!fac_rmv)
		mbhc->is_factory_mode = true;
	else
		mbhc->is_factory_mode = false;
#else
	mbhc->is_factory_mode = false;
#endif
	pr_debug("%s: is_factory_mode = %d\n", __func__, mbhc->is_factory_mode);
