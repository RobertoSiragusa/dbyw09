...
#ifdef CONFIG_SND_SOC_CS35LXX
	if (hw_get_smartpa_type() == CS35LXX) {
...
#endif
#ifdef CONFIG_SND_SOC_TFA98XX
	if (hw_get_smartpa_type() == TFA98XX) {
		pr_info("TFA98XX afe callback in \n");
		if (tfa9874_make_afe_callback(data->token, data->payload, data->payload_size))
			return 0;
	}
#endif
