...
#ifdef CONFIG_USES_CUSTOM_SWITCH_FPS
#include "sde_crtc.h"
#endif
...
#ifdef CONFIG_LCD_KIT_DRIVER
	/* store sn code */
	if (lcd_panel_sncode_store() != 0)
		DSI_ERR("lcd panel sncode store failed!\n");
#endif
...
#ifdef CONFIG_USES_CUSTOM_SWITCH_FPS
// Custom Ops
static void dsi_drm_send_dfps_cmd(struct dsi_bridge *c_bridge,
				struct drm_crtc_state *crtc_state)
{
	...
	fps_rate = sde_crtc_get_property(sde_cstate, CUST_CRTC_PROP_SWITCH_FPS);
	d_info = lcd_kit_get_disp_info();
	if (fps_rate) {
		sde_cstate->property_values[CUST_CRTC_PROP_SWITCH_FPS].value = 0;
		mutex_lock(&c_bridge->display->display_lock);
		dsi_panel_acquire_panel_lock(c_bridge->display->panel);
		switch (fps_rate) {
		case DSI_FPS_30:
			rc = dsi_panel_tx_cmd_set(c_bridge->display->panel, DSI_CMD_SET_FPS_30);
			if (rc)
				DSI_ERR("[%s] failed to send fps change cmds rc=%d, rate=%d\n",
					c_bridge->display->panel->name, rc, fps_rate);
			break;
		case DSI_FPS_60:
			rc = dsi_panel_tx_cmd_set(c_bridge->display->panel, DSI_CMD_SET_FPS_60);
			if (rc)
				DSI_ERR("[%s] failed to send fps change cmds rc=%d, rate=%d\n",
					c_bridge->display->panel->name, rc, fps_rate);
			break;
		case DSI_FPS_120:
			rc = dsi_panel_tx_cmd_set(c_bridge->display->panel, DSI_CMD_SET_FPS_120);
			if (rc)
				DSI_ERR("[%s] failed to send fps change cmds rc=%d, rate=%d\n",
					c_bridge->display->panel->name, rc, fps_rate);
			break;
		...
		if ((!rc) && (d_info != NULL))
			d_info->fps.current_fps = fps_rate;

		DSI_INFO("switch fps: %d\n", fps_rate);
	}
}
#endif
...
#ifdef CONFIG_USES_CUSTOM_SWITCH_FPS
	// Custom Ops
	if (display->panel && (display->panel->dfps_caps.dfps_support))
		dsi_drm_send_dfps_cmd(c_bridge, crtc_state);
#endif
	/*
