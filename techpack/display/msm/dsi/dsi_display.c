...
#ifdef CONFIG_LCD_KIT_DRIVER
unsigned int esd_recovery_status = 0;
unsigned int esd_recovery_backlight_status = 0;
	...
	char read_value, char expect_value)
{
	if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
		dsm_client_record(lcd_dclient,
			"lcd esd register status error:read_value=0x%x,expect_value=0x%x\n",
			read_value,
			expect_value);
...
#ifdef CONFIG_LCD_KIT_DRIVER
	/* DMS not supported on first frame, so force to 60Hz on first frame */
	if (display->is_cont_splash_enabled) {
		if ((display->config.panel_mode == DSI_OP_VIDEO_MODE) &&
			(mode->dsi_mode_flags & DSI_MODE_FLAG_DMS)) {
			DSI_INFO("DMS not supported on first frame, force to 60hz\n");
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) {
		...
		if (esd_recovery_status) {
			esd_recovery_backlight_status = 1;
			esd_recovery_status = 0;
		}
#else
