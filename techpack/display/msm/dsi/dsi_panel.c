...
#endif
#if defined(CONFIG_LOG_JANK) && defined(CONFIG_LCD_KIT_DRIVER)
...
#ifdef CONFIG_LCD_KIT_DRIVER
extern unsigned int esd_recovery_backlight_status;
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_I2C_IC:
		if (esd_recovery_backlight_status) {
			DSI_INFO("Esd recovery backlight(%d)\n", esd_recovery_level);
			rc = lcd_kit_bl_ic_set_backlight(esd_recovery_level);
			esd_recovery_backlight_status = 0;
		} else {
	...
	case DSI_BACKLIGHT_DCS_I2C_IC:
		if (esd_recovery_backlight_status) {
			DSI_INFO("Esd recovery backlight(%d)\n", esd_recovery_level);
			rc = lcd_kit_dsi_panel_update_backlight(panel, esd_recovery_level);
			rc = lcd_kit_bl_ic_set_backlight(bl_lvl);
			esd_recovery_backlight_status = 0;
		} else {
			rc = lcd_kit_dsi_panel_update_backlight(panel, bl_lvl);
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-to-30-cmds",
	"qcom,mdss-dsi-to-60-cmds",
	"qcom,mdss-dsi-to-120-cmds",
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-to-30-cmds-status",
	"qcom,mdss-dsi-to-60-cmds-status",
	"qcom,mdss-dsi-to-120-cmds-status",
};
