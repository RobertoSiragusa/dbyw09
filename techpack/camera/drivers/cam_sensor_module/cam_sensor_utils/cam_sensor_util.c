...
static uint8_t cam_power_ref = 0;
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
			...
			if (rc) {
				...
				/* In dualcam mode, if auxcam has the same
				 * gpio with maincam, the auxcam will request
				 * fail. Ignore it.
				 */
				rc = 0;
			}
	...
	struct msm_camera_gpio_num_info **pgpio_num_info)
{
	...
	*pgpio_num_info = kzalloc(sizeof(struct msm_camera_gpio_num_info),
	...
	if (rc != -EINVAL) {
		...
		CAM_INFO(CAM_SENSOR, "gpio-vana %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VANA]);
	}
	...
	if (rc != -EINVAL) {
		...
		CAM_INFO(CAM_SENSOR, "gpio-vdig %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VDIG]);
	}
	...
	if (rc != -EINVAL) {
		...
		CAM_INFO(CAM_SENSOR, "gpio-vio %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VIO]);
	}
	...
	if (rc != -EINVAL) {
		...
		CAM_INFO(CAM_SENSOR, "gpio-vaf %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VAF]);
	}

	rc = of_property_read_u32(of_node, "gpio-vdig-ctrl", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read gpio-vdig-ctrl failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-vdig-ctrl invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VDIG_CTRL] =
			gconf->cam_gpio_common_tbl[val].gpio;
		gpio_num_info->valid[SENSOR_CUSTOM_GPIO_VDIG_CTRL] = 1;

		CAM_INFO(CAM_SENSOR, "gpio-vdig-ctrl %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VDIG_CTRL]);
	} else {
		rc = 0;
	}

	rc = of_property_read_u32(of_node, "gpio-custom-vana-ref", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"read gpio-custom-vana-ref failed rc %d", rc);
			goto free_gpio_info;
		} else if (val >= gpio_array_size) {
			CAM_ERR(CAM_SENSOR, "gpio-custom-vana-ref invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VANA_REF] =
			gconf->cam_gpio_common_tbl[val].gpio;
		gpio_num_info->valid[SENSOR_CUSTOM_GPIO_VANA_REF] = 1;

		CAM_INFO(CAM_SENSOR, "gpio-custom-vana-ref %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO_VANA_REF]);
	} else {
		rc = 0;
	}
...
void cam_sensor_core_power_ref_init(void)
{
	CAM_INFO(CAM_SENSOR, "Enter");
	if (cam_power_ref != 0) {
		CAM_INFO(CAM_SENSOR, "cam power ref is %d", cam_power_ref);
		cam_power_ref = 0;
	}
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		CAM_INFO(CAM_SENSOR, "seq_type %d", power_setting->seq_type);
		...
		case SENSOR_MCLK:
			...
			break;
		case SENSOR_CUSTOM_GPIO_VANA_REF:
			cam_power_ref++;
			/* fall-through */
		case SENSOR_RESET:
		case SENSOR_STANDBY:
		...
		case SENSOR_CUSTOM_GPIO_VDIG_CTRL:
