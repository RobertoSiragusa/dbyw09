	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->sensordata->slave_info.custom_probe_info =
		probe_info->custom_probe_info;
...
int cam_module_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (!(slave_info->custom_probe_info.module_code_support)) {
	...
	s_ctrl->io_master_info.cci_client->sid =
		slave_info->custom_probe_info.eeprom_slave_addr >> 1;

	rc = camera_io_dev_read(
		&(s_ctrl->io_master_info),
		slave_info->custom_probe_info.module_code_addr,
		&module_code,
		slave_info->custom_probe_info.module_code_addr_type,
		slave_info->custom_probe_info.module_code_data_type);

	// After I/O read, restore the SID value.
	s_ctrl->io_master_info.cci_client->sid = tmp_sid;

	if (module_code != slave_info->custom_probe_info.module_code_data) {
		CAM_WARN(CAM_SENSOR, "read module code:0x%x expected module code:0x%x",
			module_code, slave_info->custom_probe_info.module_code_data);
		return -ENODEV;
	}
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		cam_sensor_core_power_ref_init();
		s_ctrl->sensor_state = CAM_SENSOR_INIT;
	}
		break;
