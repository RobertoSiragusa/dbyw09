				      ...
				      bool force_reclaim)
{
	...
	while (!list_empty(page_list)) {
		...
		if (unlikely(pagecache_dump & BIT_MM_SHRINK_INACTIVE_DUMP)) {
			...
			if (file_map)
				cur_dentry =
				(d_mapping && d_mapping->host) ? d_find_alias(d_mapping->host) : NULL;
		}
#endif
...
{
	struct lruvec *target_lruvec;
	target_lruvec = mem_cgroup_lruvec(pgdat, target_memcg);
	target_lruvec->refaults[0] =
			lruvec_page_state(target_lruvec, WORKINGSET_ACTIVATE_ANON);
	target_lruvec->refaults[1] =
