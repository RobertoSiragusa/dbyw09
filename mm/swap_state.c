...
#endif
{
	...
	for (i = 0; i < nr; i++) {
#ifdef CONFIG_REFAULT_IO_VMSCAN
		error = __radix_tree_create(&address_space->i_pages, idx + i, 0, &node, &slot);
		if (unlikely(error))
			break;
		if (*slot) {
			void *p;
			p = radix_tree_deref_slot_protected(slot, &address_space->i_pages.xa_lock);
		...
		__radix_tree_replace(&address_space->i_pages, node, slot, page + i,
					workingset_lookup_update(address_space));
#else
		set_page_private(page + i, entry.val + i);
...
#endif
{
	...
	for (i = 0; i < nr; i++) {
		...
		void **slot = NULL;
		struct radix_tree_node *node = NULL;
		__radix_tree_lookup(&address_space->i_pages, idx + i,
					&node, &slot);
		VM_BUG_ON_PAGE(!node && nr != 1, page);

		radix_tree_clear_tags(&address_space->i_pages, node, slot);
		__radix_tree_replace(&address_space->i_pages, node, slot, shadow,
				workingset_lookup_update(address_space));
#else
		radix_tree_delete(&address_space->i_pages, idx + i);
#endif
