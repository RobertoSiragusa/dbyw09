...
static int get_target_vma_hash(int measure_libs, const process_msg_t *tmsg,
			struct task_struct *target_proc,
			struct hashgen_handle *result_e_hash)
{
	int ret = -1;
			...
			struct task_struct *target_proc, int digest_size)
{
	...
	ret = get_target_vma_hash(measure_libs, tmsg,
				target_proc, result_e_hash);
