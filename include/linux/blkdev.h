...
typedef int (*lld_dev_config_mapping_partition_fn)(
		struct request_queue *, struct stor_dev_mapping_partition *);
typedef int (*lld_dev_fs_sync_done_fn)(
	struct request_queue *);
typedef int (*lld_dev_data_move_fn)(
	struct request_queue *, struct stor_dev_data_move_info *);
...
typedef int (*lld_dev_read_op_size_fn)(
	struct request_queue *, int *);
...
void ufs_mq_panic_dump_request_unistore(
...
#if defined(CONFIG_MAS_DEBUG_FS) || defined(CONFIG_MAS_BLK_DEBUG)
int mas_blk_bad_block_error_inject(
...
unsigned char mas_blk_req_get_order_nr_unistore(
	struct request *req, unsigned char stream_type, bool extern_protect);
