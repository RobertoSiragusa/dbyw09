...
#ifdef CONFIG_HW_SCHED_WALT_DEBUG
...
TRACE_EVENT(sched_load_cfs_rq,
	...
	TP_fast_assign(
		...
		if (&cfs_rq->rq->cfs == cfs_rq) {
			walt_util(__entry->util_walt, cfs_rq->rq->prev_runnable_sum);
...
#ifdef CONFIG_HW_SCHED_WALT_DEBUG
	TP_printk("cpu=%d path=%s load=%lu rbl_load=%lu util=%lu util_pelt=%lu util_walt=%lu",
#else
	TP_printk("cpu=%d path=%s load=%lu rbl_load=%lu util=%lu",
#endif
		  __entry->cpu, __get_str(path), __entry->load,
#ifdef CONFIG_HW_SCHED_WALT_DEBUG
		  __entry->rbl_load, __entry->util,__entry->util_pelt, __entry->util_walt)
#else
...
#ifdef CONFIG_HW_SCHED_WALT_DEBUG
		__entry->util_pelt = se->avg.util_avg;
		if(!se->my_q) {
...
#ifdef CONFIG_HW_SCHED_WALT_DEBUG
		  __entry->load, __entry->rbl_load, __entry->util, __entry->util_pelt, __entry->util_walt)
#else
