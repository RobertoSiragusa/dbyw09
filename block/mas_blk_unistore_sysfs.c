	...
	struct request_queue *q, const char *page, size_t count)
{
	...
	unsigned long val;
	struct blk_dev_lld *lld_fn = mas_blk_get_lld(q);
	struct stor_dev_mapping_partition *mapping_info = NULL;

	ret = queue_var_store(&val, page, count);
	if (ret < 0)
		return count;

	mapping_info = kmalloc(sizeof(struct stor_dev_mapping_partition), GFP_KERNEL);

	pr_err("%s 2M map area begin from %llu\n", __func__, val);

	if (!lld_fn || !lld_fn->unistore_ops.dev_config_mapping_partition)
		return count;
	...
	struct request_queue *q, const char *page, size_t count)
{
	...
	pr_err("%s rescue_block_inject_data! <lba:0x%x> <ret:%d>\n", __func__, val, re);
	...
	struct stor_dev_data_move_info *data_move_info, unsigned long val)
{
	...
	if (val && lld_fn && lld_fn->unistore_ops.dev_data_move) {
		...
		pr_err("%s <data_move_source_addr:0x%x> <ret:%d>\n", __func__, val, ret);
	...
	struct request_queue *q, const char *page, size_t count)
{
	...
	if (val < 0 || val > DATA_MOVE_MAX_NUM)
		return count;
