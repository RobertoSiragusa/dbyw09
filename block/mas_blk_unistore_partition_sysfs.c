...
static void bdev_issue_discard(struct block_device *bdev, void *arg)
{
	...
	blkdev_issue_discard(bdev, (para->lba >> SECTOR_BYTE),
...
ssize_t mas_queue_wo_abnormal_tst_enable_store(
	struct request_queue *q, const char *page, size_t count)
{
	...
	if (ret >= 0) {
		if (val) {
			pr_err("%s: wo abnormal tst start\n", __func__);
			blk_order_nr_reset(q->tag_set);
		}
	}
	return (ssize_t)count;
}

ssize_t mas_queue_wo_panic_tst_enable_store(
	struct request_queue *q, const char *page, size_t count)
{
	ssize_t ret;
	unsigned long val;
	...
	if (ret >= 0) {
		if (val == 0xff) {
			pr_err("%s: wo panic tst start\n", __func__);
			mas_blk_rdr_panic("wo panic tst");
		}
	}
	return (ssize_t)count;
}
