...
int get_wifi_para_for_booster(struct wifi_info *w_info, int w_info_len)
{
	...
	for (i = 0; i < w_info_len; i++) {
		struct cfg80211_registered_device *tmp_rdev = g_wifi_dev[i].rdev;
		struct net_device *tmp_dev = g_wifi_dev[i].dev;
		u8 *tmp_mac_addr = g_wifi_dev[i].mac_addr;
		if (g_wifi_dev[i].wifi_state_flag == 1 && tmp_rdev != NULL && tmp_dev != NULL && tmp_mac_addr != NULL) {
			struct station_info s_info;
			if (memset_s(&s_info, sizeof(s_info), 0, sizeof(s_info)) != EOK)
				printk(KERN_DEBUG "fail to get station info.");
			ret = rdev_get_station(tmp_rdev, tmp_dev, tmp_mac_addr, &s_info);
			if (ret)
				return ret;
			if (memcpy_s(w_info[i].name, sizeof(w_info[i].name), tmp_dev->name, sizeof(tmp_dev->name)) != EOK)
				printk(KERN_DEBUG "fail to get net device.");
			w_info[i].flag = 1;
			if (s_info.filled & (BIT(NL80211_STA_INFO_RX_BYTES) | BIT(NL80211_STA_INFO_RX_BYTES64)))
				w_info[i].rx_bytes = (u32)s_info.rx_bytes;
			if (s_info.filled & (BIT(NL80211_STA_INFO_TX_BYTES) | BIT(NL80211_STA_INFO_TX_BYTES64)))
			...
			if (s_info.filled & BIT(NL80211_STA_INFO_TX_BITRATE))
				w_info[i].txrate = cfg80211_calculate_bitrate(&s_info.txrate);
			if (s_info.filled & BIT(NL80211_STA_INFO_RX_BITRATE))
				w_info[i].rxrate = cfg80211_calculate_bitrate(&s_info.rxrate);
	...
	if (g_wifi_dev[0].wifi_state_flag == 1 || g_wifi_dev[1].wifi_state_flag == 1)
		return 0;
	else
		return -1;
}
				...
				struct net_device *dev, u8 *mac_addr)
{
	if (strstr(name, "wlan0")) {
		...
		if (memcpy_s(g_wifi_dev[0].mac_addr, sizeof(g_wifi_dev[0].mac_addr), mac_addr, MAC_ADDR_SIZE * sizeof(u8)) != EOK)
			printk(KERN_DEBUG "fail to get net connect wlan0.");
	}
	if (strstr(name, "wlan1")) {
		g_wifi_dev[1].wifi_state_flag = 1;
		g_wifi_dev[1].rdev = rdev;
		g_wifi_dev[1].dev = dev;
		if (memcpy_s(g_wifi_dev[1].mac_addr, sizeof(g_wifi_dev[1].mac_addr), mac_addr, MAC_ADDR_SIZE * sizeof(u8)) != EOK)
			printk(KERN_DEBUG "fail to get net connect wlan1.");
	}
}
#endif
...
static void set_wifi_state_disconnect(const char *name)
{
	if (strstr(name, "wlan0")) {
		...
		if (memset_s(g_wifi_dev[0].mac_addr, sizeof(g_wifi_dev[0].mac_addr), 0, MAC_ADDR_SIZE * sizeof(u8)) != EOK)
			printk(KERN_DEBUG "fail to get disconnect wlan0.");
	}
	if (strstr(name, "wlan1")) {
		g_wifi_dev[1].wifi_state_flag = 0;
		g_wifi_dev[1].rdev = NULL;
		g_wifi_dev[1].dev = NULL;
		if (memset_s(g_wifi_dev[1].mac_addr, sizeof(g_wifi_dev[1].mac_addr), 0, MAC_ADDR_SIZE * sizeof(u8)) != EOK)
			printk(KERN_DEBUG "fail to get disconnect wlan1.");
	}
}
#endif
