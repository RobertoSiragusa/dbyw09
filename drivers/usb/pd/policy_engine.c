...
#define PD_MAX_SINK_CURRENT	5000
#define PD_REQUEST_VOLTEGE	1000

#define ICON_TYPE_QUICK     2
#define QUICK_CHG_VOL_TH    9000000
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	struct notifier_block	pd_event_nb;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define CC_NOT_CONNECTED       3
static struct device *g_typec_dev;
struct usbpd *g_pd;
static bool g_pd_finish_flag;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
{
	enum plug_orientation cc_orientation = usbpd_get_plug_orientation(g_pd);

	if (cc_orientation == ORIENTATION_NONE)
...
static void sink_cap_match_src_cap(
	struct usbpd *pd, int mv, int ma, int *pos)
{
	int src_mv;
	int src_ma;
	int max_cur;
	int j;
	int cur = PD_MAX_SINK_CURRENT;
	bool mismatch = false;

	for (j = 0; j < PD_MAX_DATA_OBJ; j++) {
		if (PD_SRC_PDO_TYPE(pd->received_pdos[j]) !=
			PD_SRC_PDO_TYPE_FIXED)
			continue;
		src_mv = PD_SRC_PDO_FIXED_VOLTAGE(pd->received_pdos[j]);
		src_ma = PD_SRC_PDO_FIXED_MAX_CURR(pd->received_pdos[j]);
		if (mv != src_mv)
			continue;
		if (cur > min(ma, src_ma)) {
			cur = max_cur = min(ma, src_ma);
			/* current unit : 10mv */
			if ((cur * 10) < PD_MIN_SINK_CURRENT) {
				mismatch = true;
				max_cur = PD_MIN_SINK_CURRENT / 10;
			}
			*pos = j + 1;
			pd->requested_pdo = *pos;
			pd->requested_current = cur * 10;
			/* voltage unit : 50mv */
			pd->requested_voltage = src_mv * 50 * PD_REQUEST_VOLTEGE;
			pd->rdo = PD_RDO_FIXED(*pos, 0, mismatch, 1, 1,
				cur, max_cur);
		}
	}
}

static int pd_select_default_pdo(struct usbpd *pd)
{
	int sink_mv;
	int sink_ma;
	int i;
	...
	for (i = (pd->num_sink_caps - 1); i >= 0; i--) {
		sink_mv = PD_SRC_PDO_FIXED_VOLTAGE(pd->sink_caps[i]);
		sink_ma = PD_SRC_PDO_FIXED_MAX_CURR(pd->sink_caps[i]);
		usbpd_dbg(&pd->dev, "sink mv = %d, ma=%d\n", sink_mv, sink_ma);
		sink_cap_match_src_cap(pd, sink_mv, sink_ma, &pos);
		if (pos != 0)
			break;
	}
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (pd->requested_voltage >= QUICK_CHG_VOL_TH)
		wired_connect_send_icon_uevent(ICON_TYPE_QUICK);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	if (pos == 0) {
		usbpd_err(&pd->dev, "source cap does not match the sink cap\n");
		return -EINVAL;
	}
...
static int pd_eval_src_caps(struct usbpd *pd)
{
	...
	/* Select the max PDO of sink cap immediately. */
	if (pd_select_default_pdo(pd))
		return -EINVAL;
...
#ifndef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	usbpd_info(&pd->dev, "check vbus after enable\n");
	count = 10;
	/*
...
static void enter_state_snk_ready(struct usbpd *pd)
{
	...
	power_event_bnc_notify(POWER_BNT_THIRDPLAT_PD,
		POWER_NE_THIRDPLAT_PD_START, NULL);
	pd_dpm_set_ctc_cable_flag(true);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static int pd_event_notifier_call(struct notifier_block *nb,
	unsigned long event, void *data)
{
	switch (event) {
	case POWER_NE_THIRDPLAT_PD_START:
		g_pd_finish_flag = true;
		break;
	case POWER_NE_THIRDPLAT_PD_STOP:
		g_pd_finish_flag = false;
		break;
	default:
		break;
	}
...
struct usbpd *usbpd_create(struct device *parent)
{
	...
	usbpd_info(&pd->dev, "usbpd_create get usb\n");
	...
	usbpd_info(&pd->dev, "usbpd_create get battery\n");
	if (!pd->bat_psy)
#if defined(CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION) && \
	defined(CONFIG_HUAWEI_MIXED_BATT)
		pd->bat_psy = power_supply_get_by_name("bk_battery");
#else
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pd->pd_event_nb.notifier_call = pd_event_notifier_call;
	ret = power_event_bnc_register(POWER_BNT_THIRDPLAT_PD, &pd->pd_event_nb);
	if (ret < 0)
		goto del_inst;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
void usbpd_destroy(struct usbpd *pd)
{
	...
	list_del(&pd->instance);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	power_event_bnc_unregister(POWER_BNT_THIRDPLAT_PD, &pd->pd_event_nb);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	g_typec_dev = power_sysfs_create_group("hw_typec", "typec",
		&pd_dpm_attr_group);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
