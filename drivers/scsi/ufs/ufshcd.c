	...
	enum dma_data_direction cmd_dir)
{
	...
	if (unlikely(lrbp->cmd && lrbp->cmd->request &&
		     lrbp->cmd->request->cmd_flags & REQ_CP))
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	if (unlikely(cmd->cmnd[0] == WRITE_BUFFER &&
		...
		dev_err(hba->dev, "%s: FFU process, wait_for_ufs_all_complete:%d",
			__func__, err);
	}
	if (!(host->queue_quirk_flag &
	      SHOST_QUIRK(SHOST_QUIRK_DRIVER_TAG_ALLOC)))
...
void ufs_get_geometry_info(struct ufs_hba *hba)
{
	...
	b_max_number_lu = desc_buf[GEOMETRY_DESC_PARAM_MAX_NUM_LUN];
	if (b_max_number_lu == 0) {
		hba->dev_max_num_lu = 8;
	} else if (b_max_number_lu == 1) {
		hba->dev_max_num_lu = 32;
	} else {
		pr_err("unkonwn b_max_number_lu %d", b_max_number_lu);
		hba->dev_max_num_lu = UFS_DEV_MAX_NUMBER_LU;
	}
...
static int ufshcd_alloc_lu_wp(struct ufs_hba *hba)
{
	if (unlikely(!hba->lu_num || hba->lu_num > hba->dev_max_num_lu))
		return -EINVAL;
	hba->lu_wp = kzalloc(hba->lu_num * sizeof(struct ufs_lu_wp_info), GFP_KERNEL);
	if (!hba->lu_wp)
		return -ENOMEM;
	return 0;
}

static void ufshcd_get_lu_wp_info(struct ufs_hba *hba, struct scsi_device *sdev)
{
	u8 b_lu_write_protect;

	if (sdev->lun < hba->lu_num) {
		ufshcd_get_lu_wp(hba, ufshcd_scsi_to_upiu_lun(sdev->lun),
				 &b_lu_write_protect);
		hba->lu_wp[sdev->lun].lu_wp = b_lu_write_protect;
	}
}
...
static int ufshcd_slave_alloc(struct scsi_device *sdev)
{
	...
	ufshcd_get_lu_wp_info(hba, sdev);
...
bool ufshcd_wb_sup(struct ufs_hba *hba)
{
					...
					struct ufs_dev_desc *dev_desc)
{
	...
	/* Check if device is 186x, return */
	if (dev_desc->wmanufacturerid == UFS_VENDOR_HI1861)
		return 0;
			       ...
			       struct ufs_dev_desc *dev_desc)
{
	...
	u8 prl_index;
#ifdef CONFIG_HUAWEI_STORAGE_ROFA_FAULT_INJECT
	u8 life_time_buf[QUERY_DESC_MAX_SIZE];
	...
	hba->lu_num = desc_buf[DEVICE_DESC_PARAM_NUM_LU];
	/*
...
skip_unit_desc:
	...
	err = ufshcd_read_string_desc(hba, serial_num_index, desc_buf,
	...
	set_ufs_bootdevice_product_name(dev_desc->model);
	set_ufs_bootdevice_manfid(hba->manufacturer_id);

	/* Getting PRL */
	memset(desc_buf, 0, QUERY_DESC_MAX_SIZE);
	err = ufshcd_read_string_desc(hba, prl_index, desc_buf,
	...
	strlcpy(dev_desc->prl, (desc_buf + QUERY_DESC_HDR_SIZE),
		min_t(u8, desc_buf[QUERY_DESC_LENGTH_OFFSET],
			MAX_PRL_LEN));
	...
	set_ufs_fw_version(dev_desc->prl);
...
static int ufshcd_ffu_get_device_data(struct ufs_hba *hba, struct ufs_dev_desc *dev_desc)
{
        int ret = 0;
        ...
        if (dev_desc && hba->ffu_get_device_flag) {
                memset(dev_desc, 0, sizeof(struct ufs_dev_desc));
                ret = ufs_get_device_desc(hba, dev_desc);
                if (ret) {
                        dev_info(hba->dev, "%s: Failed getting device info. err = %d\n",
                                        __func__, ret);
                        return ret;
                }
                ufs_fixup_device_setup(hba, dev_desc);
...
reinit:
	...
	ufs_get_geometry_info(hba);
	if (ufshcd_alloc_lu_wp(hba)) {
		dev_err(hba->dev, "ufshcd_alloc_lu_wp fail\n");
		goto out;
	}
	...
	if (ufshcd_ffu_get_device_data(hba, &card)) {
		goto out;
	}
	...
	hufs_bootdevice_get_dieid(hba, &card);
#endif
	 ...
	 * context, no need to scan the host and init hpb
	 */
					  ...
					  unsigned int timeout)
{
	...
	spin_lock_irqsave(hba->host->host_lock, flags);
#ifdef CONFIG_MAS_SCSI_UFS_DUMP
...
static int _ufshcd_queuecommand_directly(struct ufs_hba *hba,
					  struct scsi_cmnd *cmd,
					  unsigned int timeout, int retries)
{
	int err;
...
start:
	...
	/* retry 3 times (three chances) to ignore possible Unit Attention */
	err = _ufshcd_queuecommand_directly(hba, cmd, timeout, 3);
