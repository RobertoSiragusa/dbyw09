...
#ifdef CONFIG_HUAWEI_DSM_IOMT_EMMC_HOST
	iomt_host_latency_mrq_start(cmd->mrq);
#endif
...
#ifdef CONFIG_HUAWEI_DSM_IOMT_EMMC_HOST
	iomt_host_latency_mrq_init(mrq, mmc);
#endif
...
static int sdhci_get_cd(struct mmc_host *mmc)
{
	...
	if (gpio_cd >= 0) {
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
		if (host->mmc->is_nm_card == 1) {
			if (gpio_cd == 0) {
				mutex_lock(&card_insert_muxlock);
				host->mmc->card_inserted = gpio_cd;
				host->mmc->nm_card_cmd_detect_result = 1;
				mutex_unlock(&card_insert_muxlock);
			}
			return host->mmc->card_inserted;
		}
#endif
		return !!gpio_cd;
	}
