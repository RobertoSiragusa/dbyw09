...
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
...
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
	msm_host->mmc->cd_gpio = pdata->status_gpio;
#endif
	if (gpio_is_valid(pdata->status_gpio) && !(flags & OF_GPIO_ACTIVE_LOW))
		pdata->caps2 |= MMC_CAP2_CD_ACTIVE_HIGH;

	if (of_property_read_u32(np, "nm-card", &value)) {
		pr_err("%s : nm-card property not found,"
		       "using value of 0 as default\n", __func__);
		value = 0;
	}
	msm_host->mmc->is_nm_card = value;
	pr_err("%s: mmc is_nm_card = %d\n", __func__, msm_host->mmc->is_nm_card);

#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
	msm_host->mmc->sd_1v8_enable_gpio = of_get_named_gpio_flags(np, "sd-1v8-enable-gpios", 0, NULL);
	if (!gpio_is_valid(msm_host->mmc->sd_1v8_enable_gpio))
		pr_err(" mmc1: %s: sd_1v8_enable_gpio = %d is invalid\n", __func__, msm_host->mmc->sd_1v8_enable_gpio);
#endif
...
static int sdhci_msm_vreg_enable(struct sdhci_msm_reg_data *vreg)
{
	...
	if (!strcmp(vreg->name, "vdd") && detect_nm_card_fail) {
		pr_err("%s: can't set pwr regulator_set_voltage to 2.95V", __func__);
		return ret;
	}
#endif
	...
	pr_err("%s: regulator_enable(%s) successed.\n",
			__func__, vreg->name);
...
static int sdhci_msm_vreg_disable(struct sdhci_msm_reg_data *vreg)
{
	...
	if (vreg->is_enabled && !vreg->is_always_on) {
		...
		pr_err("%s: regulator_disable(%s) successed.\n",
			__func__, vreg->name);
...
static irqreturn_t sdhci_msm_pwr_irq(int irq, void *data)
{
	...
	if (!(host->mmc->caps & MMC_CAP_NONREMOVABLE) &&
		host->mmc->is_nm_card && host->mmc->nm_card_cmd_detect_result)
...
static int sdcard_intr_gpio_read(struct seq_file *m, void *v)
{
	...
	pr_info("%s: sdcard detect gpio_value is %d\n", __func__, gpio_value);
	seq_printf(m, "%d\n", gpio_value);
...
static bool sdhci_dev_rename(struct platform_device *pdev)
{
	int ret;
	int pri_id;
	struct device *dev = &pdev->dev;
	static const char *const msmsdhc1 = "msm_sdhc1";
	static const char *const msmsdhc2 = "msm_sdhc2";

	pri_id = of_alias_get_id(dev->of_node, "sdhc");
	switch (pri_id) {
	case MMC_EMMC:
		pdev->name = msmsdhc1;
		ret = device_rename(dev, msmsdhc1);
		if (ret < 0) {
			pr_err("[sdhci] %s dev set name %s fail\n",
			       __func__, msmsdhc1);
			goto error;
		}
		break;
	case MMC_SDCARD:
		pdev->name = msmsdhc2;
		ret = device_rename(dev, msmsdhc2);
		if (ret < 0) {
			pr_err("[sdhci] %s dev set name %s fail\n",
			       __func__, msmsdhc2);
			goto error;
		...
		break;
	default:
		pr_err("[sdhci] %s pri_id is out of range\n", __func__);
		goto error;
	}

	/* still keep pdev->name same with dev->kobj.name */
	pdev->name = dev->kobj.name;
	return true;
error:
	pdev->name = dev->kobj.name;
	return false;
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	ret = sdhci_dev_rename(pdev);
	if (ret == false) {
		pr_err("[sdhci] %s rename err, use default name\n", __func__);
	}
