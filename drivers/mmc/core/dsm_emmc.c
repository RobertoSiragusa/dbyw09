...
#define EMMC_MANUFACTURER_ID_YMTC      0x9b
#define EMMC_MANFID_SHIFT_OF_RAW_CID_0 24
			...
			struct mmc_request *mrq)
{
	...
	if (host->card) {
		unsigned int manfid;
		manfid = host->card->raw_cid[0] >> EMMC_MANFID_SHIFT_OF_RAW_CID_0;
		if (manfid == EMMC_MANUFACTURER_ID_MICRON || manfid == EMMC_MANUFACTURER_ID_YMTC) {
			printk("emmc manufacturer id is xxx or xxx, exit\n");
			return;
		}
	}
