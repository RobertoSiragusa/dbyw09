...
#ifdef CONFIG_HUAWEI_DSM_IOMT_EMMC_HOST
static void iomt_host_latency_mrq_end_qcom(struct mmc_host *mmc_host, struct mmc_request *mrq)
{
	...
	if (unlikely(i >= iomt_host_info->io_timeout_dsm.judge_slot)) {
		...
		if (mrq->data) {
			io_timeout_dsm->block_ticks |=
				...
				IOMT_OP_BLOCK_SHIFT);
			io_timeout_dsm->op_arg = (unsigned int) mrq->data->blk_addr;
		}
	}
}
#endif
...
#ifdef CONFIG_HUAWEI_DSM_IOMT_EMMC_HOST
	iomt_host_latency_mrq_end_qcom(host, mrq);
#endif
	/*
...
void mmc_rescan(struct work_struct *work)
{
	...
	mmc_bus_put(host);
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
	if (mmc_card_is_removable(host) && host->is_nm_card)
		nmcard_detect_run(host);
#endif
	mmc_claim_host(host);
	if (mmc_card_is_removable(host) && host->ops->get_cd &&
			host->ops->get_cd(host) == 0) {
		if (!strcmp(mmc_hostname(host), "mmc1"))
			pr_err("%s:%s:maybe the sdcard is not present, so exit\n",
				mmc_hostname(host), __func__);
		mmc_power_off(host);
		mmc_release_host(host);
		goto out;
		...
		if (!strcmp(mmc_hostname(host), "mmc1"))
			pr_err("%s:%s:sdcard is present, prepare to init sdcard\n",
				mmc_hostname(host), __func__);
	}
 ...
 out:
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
	if (extend_wakelock && mmc_card_is_removable(host) && host->is_nm_card) {
		pr_err("%s:%s %s_detect wakelock hold\n", mmc_hostname(host), __func__,
				mmc_hostname(host));
