...
void stub_mmc_to_adapt_ufs(struct device *dev)
{
	int ida_num;

	/* Prerequisites: devices load Queue: 1.emmc, 2.SD, 3.other devices */
	if (!strncmp(dev_name(dev), "msm_sdhc2", strlen("msm_sdhc2"))) {
		/* last ida number is current |ida_num -1|, ida number start from 0 */
		ida_num = ida_simple_get(&mmc_host_ida, 0, 0, GFP_KERNEL);
		if (ida_num == 0) {
			pr_info("[mmc] ufs boot, to do one stub\n");
		} else if (ida_num == 1) {
			ida_simple_remove(&mmc_host_ida, ida_num);
			pr_info("[mmc] emmc boot, no need stub\n");
		}
	} else {
		pr_info("[mmc] no need stub\n");
	}
}
...
#ifdef CONFIG_HUAWEI_WIFIONLY_NMCARD
	/* config nm_card_cmd_detect_result */
	host->nm_card_cmd_detect_result = 1;
#endif
