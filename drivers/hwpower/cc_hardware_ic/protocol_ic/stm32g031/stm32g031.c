...
static void stm32g031_ops_register(struct stm32g031_device_info *di)
{
	if (stm32g031_is_support_scp(di))
		stm32g031_scp_protocol_register(di);

	if (stm32g031_is_support_fcp(di))
		stm32g031_fcp_protocol_register(di);

	if (stm32g031_is_support_ufcs(di))
		stm32g031_ufcs_protocol_register(di);
	...
	const struct i2c_device_id *id)
{
	...
	if (stm32g031_fw_update_empty_mtp(di))
		goto fail_free_gpio;

	/* need update mtp when mtp is latest */
	if (stm32g031_fw_update_latest_mtp(di))
		goto fail_free_gpio;

	if (stm32g031_fw_get_dev_id(di))
		goto fail_free_gpio;
