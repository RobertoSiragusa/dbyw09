...
static int stm32g031_ufcs_detect_adapter(void *dev_data)
{
	...
	if (i == STM32G031_UFCS_HANDSHARK_RETRY_CYCLE) {
		...
		return UFCS_DETECT_FAIL;
	}

	hwlog_info("handshake succ\n");
	return UFCS_DETECT_SUCC;
}
...
static int stm32g031_ufcs_send_msg_head(void *dev_data, u8 *data, u8 len)
{
	...
	if (len != sizeof(struct ufcs_protocol_header_data)) {
...
{
	struct stm32g031_device_info *di = dev_data;
	u8 tmp_len = sizeof(struct ufcs_protocol_header_data);
...
static int stm32g031_ufcs_master_reset(void *dev_data)
{
	struct stm32g031_device_info *di = dev_data;
...
static struct ufcs_protocol_ops stm32g031_ufcs_protocol_ops = {
	.chip_name = "stm32g031",
	...
	.soft_reset_master = stm32g031_ufcs_master_reset,
};

int stm32g031_ufcs_protocol_register(struct stm32g031_device_info *di)
{
	stm32g031_ufcs_protocol_ops.dev_data = (void *)di;
	return ufcs_protocol_ops_register(&stm32g031_ufcs_protocol_ops);
}
