...
static void sm5450_init_ops_dev_data(struct sm5450_device_info *di)
{
	...
	if (!di->ic_role) {
		snprintf(di->name, SM5450_DEV_NAME_LEN, "sm5450");
	} else {
		snprintf(di->name, SM5450_DEV_NAME_LEN, "sm5450_%d", di->ic_role);
...
static void sm5450_ops_register(struct sm5450_device_info *di)
{
	...
	ret += dc_batinfo_ops_register(LVC_MODE, di->ic_role, &di->batinfo_ops);
	if (ret)
