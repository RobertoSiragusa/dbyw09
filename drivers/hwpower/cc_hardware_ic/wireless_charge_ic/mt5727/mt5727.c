...
{
	return mt5727_read_block(di, reg, data, POWER_BYTES_LEN);
}

int mt5727_write_byte(struct mt5727_dev_info *di, u16 reg, u8 data)
{
	return mt5727_write_block(di, reg, &data, POWER_BYTES_LEN);
}

int mt5727_read_word(struct mt5727_dev_info *di, u16 reg, u16 *data)
{
	u8 buff[POWER_BYTES_PER_WORD] = { 0 };

	if (!data || mt5727_read_block(di, reg, buff, POWER_BYTES_PER_WORD))
		return -EINVAL;
...
{
	u8 buff[POWER_BYTES_PER_WORD];

	buff[0] = data & POWER_MASK_BYTE;
	buff[1] = data >> POWER_BITS_PER_BYTE;

	return mt5727_write_block(di, reg, buff, POWER_BYTES_PER_WORD);
}

int mt5727_read_dword(struct mt5727_dev_info *di, u16 reg, u32 *data)
{
	u8 buff[POWER_BYTES_PER_DWORD] = { 0 };

	if (!data || mt5727_read_block(di, reg, buff, POWER_BYTES_PER_DWORD))
		return -EINVAL;
...
int mt5727_write_dword(struct mt5727_dev_info *di, u16 reg, u32 data)
{
	...
	return mt5727_write_block(di, reg, buff, POWER_BYTES_PER_DWORD);
}
