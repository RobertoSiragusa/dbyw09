...
static struct dc_batinfo_ops *g_sc4_batinfo_ops[CHARGE_IC_MAX_NUM];
...
struct dc_batinfo_ops *dc_ic_get_battinfo_ops(int mode, unsigned int index)
{
	...
	case SC4_MODE:
		return g_sc4_batinfo_ops[index];
	default:
		return NULL;
	}
}
...
static void dc_ic_parse_dts(struct device_node *np, struct dc_ic_dev *di)
{
	...
	di->para_flag = true;
	power_gpio_config_output(np, "vbat_en_gpio", "vbat_en_gpio", &di->vbat_en_gpio, 1);
}
