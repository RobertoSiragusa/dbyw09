...
static int scp_protocol_auth_encrypt_start(int key)
{
	int ret;
	...
	scp_auth_clean_hash_data();
	if (scp_protocol_copy_hash_value(key, scp_auth_get_hash_data_header(),
		scp_auth_get_hash_data_size())) {
		ret = -1;
		goto fail_encrypt;
	}

	/* eighth: wait hash calculate complete */
	ret = scp_auth_wait_completion();
