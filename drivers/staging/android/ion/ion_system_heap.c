...
static int ion_sys_pool_init(struct ion_system_heap *heap, int (*threadfn)(void *))
{
	atomic_set(&heap->wait_flag, 0);
	init_waitqueue_head(&heap->sys_pool_wait);
	heap->sys_pool_thread = kthread_run(threadfn, heap,
						"%s", "sys_pool");
...
struct ion_heap *ion_system_heap_create(struct ion_platform_heap *data)
{
	...
	if (ion_sys_pool_init(heap, ion_sys_pool_kthread))
		goto destroy_pools;
	return &heap->heap;
destroy_pools:
