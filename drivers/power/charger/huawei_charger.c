...
static void update_iin_thermal(struct charge_device_info *di)
{
	...
	power_supply_set_int_property_value_with_psy(di->chg_psy,
		POWER_SUPPLY_PROP_IIN_THERMAL, di->sysfs_data.iin_thl);
...
#if IS_ENABLED(CONFIG_QTI_PMIC_GLINK)
		/* 1: valid buff size */
		if (power_glink_set_property_value(POWER_GLINK_PROP_ID_SET_CHARGE_ENABLE, (unsigned int *)&val, 1))
			return -EFAULT;
#else
		if (!power_supply_check_psy_available("battery", &di->batt_psy))
			return -EFAULT;
...
static int dcp_set_enable_charger(unsigned int val)
{
	...
	/* 1: valid buff size */
	if (power_glink_set_property_value(id, &val, 1))
		return -EFAULT;
...
static int dcp_set_ichg_limit(unsigned int val)
{
	...
	return power_supply_set_int_property_value_with_psy(di->chg_psy,
		POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX, (int)(val * 1000));
}
...
static int huawei_charger_value_dump(char *buffer, int size, void *dev_data)
{
	...
	char c_type[CHG_LOG_STR_LEN] = {0};
	char c_status[CHG_LOG_STR_LEN] = {0};
	char c_health[CHG_LOG_STR_LEN] = {0};
	char c_on[CHG_LOG_STR_LEN] = {0};
	...
	if (power_supply_check_psy_available("usb", &di->usb_psy)) {
		...
		d_data.in_type = power_supply_return_int_property_value_with_psy(di->usb_psy,
			POWER_SUPPLY_PROP_TYPE);
		conver_usbtype(d_data.in_type, c_type);
		d_data.usb_vol = power_supply_app_get_usb_voltage_now();
	}
	if (power_supply_check_psy_available("battery", &di->batt_psy)) {
		d_data.ch_en = power_supply_return_int_property_value_with_psy(di->batt_psy,
			POWER_SUPPLY_PROP_CHARGING_ENABLED);
		d_data.status = power_supply_return_int_property_value_with_psy(di->batt_psy,
			POWER_SUPPLY_PROP_STATUS);
		conver_charging_status(d_data.status, c_status);
		d_data.health = power_supply_return_int_property_value_with_psy(di->batt_psy,
			POWER_SUPPLY_PROP_HEALTH);
		conver_charger_health(d_data.health, c_health);
	...
	(void)snprintf(buffer, size, "%-6d %-10s  %-7d"
		...
		"  %-7d  %-4d  %-6d  %-7d  %-10d"
		"  %-4d  %-8d  %-7d  %-4s",
		d_data.online, c_type, d_data.usb_vol,
		di->sysfs_data.iin_thl, d_data.ch_en, c_status, c_health,
		d_data.bat_present, d_data.temp, d_data.real_temp, d_data.vol,
		d_data.cur, d_data.capacity, d_data.real_soc, d_data.ibus, c_on);
	return 0;
}
...
static int huawei_charger_get_log_head(char *buf, int size, void *dev_data)
{
	...
	(void)snprintf(buf, size, "online in_type     usb_vol"
		...
		"  present  temp  temp_r  vol      cur"
		"         cap   real_soc  ibus     mode");
	return 0;
}
