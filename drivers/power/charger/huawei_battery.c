...
static int set_prop_iin_thermal(struct huawei_battery_info *info, int value)
{
	...
	if (!info->ibus_detecting)
		rc = vote(info->usb_icl_votable, IN_THERMAL_VOTER, true,
			info->thermal_limit_current);
	return rc;
}
	...
	enum power_supply_property prop, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_WLRX_ICL:
		rc = vote(info->usb_icl_votable, ICL_WLRX_VOTER, true, val->intval);
		goto OUT;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_BASP:
		rc = vote(info->vbat_votable, VBAT_BASP_VOTER,
			true, val->intval / POWER_UV_PER_MV);
		goto OUT;
...
static int huawei_battery_probe(struct platform_device *pdev)
{
	...
	info->fcc_votable = create_votable("BATT: fcc", VOTE_MIN,
	...
	info->usb_icl_votable = create_votable("BATT: usb_icl", VOTE_MIN,
	...
	info->vbat_votable = create_votable("BATT: vbat", VOTE_MIN,
		set_vbat_cb, info);
	...
	info->bch_votable = create_votable("BATT: bch", VOTE_SET_ANY,
	...
	info->chr_type = power_platform_get_charger_type();
	if (info->chr_type == CHARGER_TYPE_STANDARD)
