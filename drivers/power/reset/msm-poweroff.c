...
{
	int ret;
	if (ksys_access(USB_REBOOT_NODE, 0) != 0) {
		pr_err("access %s fail, try to make\n", USB_REBOOT_NODE);
...
int usb_update_thread(void *__unused)
{
	...
	pr_err("usb_update_thread begin\n");
	for (;;) {
		...
		if (dload_magic == SDUPDATE_FLAG_MAGIC_NUM) {
...
#else
#ifdef CONFIG_RAINBOW_HIMNTN
		/* Both can only be set if has Fastboot dump flag, otherwise mini */
		if (cmd_himntn_item_switch(HIMNTN_ID_FASTBOOT_SWITCH)) {
			if (!msm_minidump_enabled()) {
				pr_err("Minidump not enabled, setting fulldump only\n");
				dload_type = SCM_DLOAD_FULLDUMP;
