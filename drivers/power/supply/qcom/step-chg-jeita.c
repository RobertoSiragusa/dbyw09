...
#define HLTHERM_CURRENT_LOW_TEMP 2000000
#define HLTHERM_MIN_LOW_TEMP     (-200)
#define HLTHERM_MAX_HIGH_TEMP    800
#define HLTHERM_T1_TO_T2         0
#define HLTHERM_T2_TO_T3         1
#define HLTHERM_T4_TO_T5         3
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#ifdef CONFIG_HLTHERM_RUNTEST
	max_fv_uv = HLTHERM_RUNTEST_MAX_VOL;
#endif /* CONFIG_HLTHERM_RUNTEST */
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#ifdef CONFIG_HLTHERM_RUNTEST
	/* config threshold:hard hot = 60, hard cold = -20 */
	chip->jeita_fcc_config->fcc_cfg[HLTHERM_T1_TO_T2].low_threshold =
		HLTHERM_MIN_LOW_TEMP;
	chip->jeita_fcc_config->fcc_cfg[HLTHERM_T1_TO_T2].value =
		HLTHERM_CURRENT_LOW_TEMP;
	chip->jeita_fcc_config->fcc_cfg[HLTHERM_T2_TO_T3].value =
		HLTHERM_CURRENT_LOW_TEMP;
	chip->jeita_fcc_config->fcc_cfg[HLTHERM_T4_TO_T5].high_threshold =
		HLTHERM_MAX_HIGH_TEMP;
#endif /* CONFIG_HLTHERM_RUNTEST */
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	if (chip->jeita_fcc_config->param.use_bms) {
		...
		if (!bk_batt_psy)
			pr_err("Couldn't read bk_batt_psy\n");
		else
			rc = power_supply_get_property(bk_batt_psy,
				chip->jeita_fcc_config->param.psy_prop, &pval);
	}
#else
	if (chip->jeita_fcc_config->param.use_bms)
