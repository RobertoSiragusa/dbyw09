...
#define TYPE_C_DEBUG_CABLE_CHARGING_ENABLE 0x17
static struct smb_charger *g_chg;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->usb_third_bc_12 = of_property_read_bool(node, "usb-thd-bc-12");
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	rc = of_property_read_u32(node, "qcom,dynamic-cv-flag", &temp);
	if (rc < 0)
		chg->dynamic_cv_flag = 0;
	else
		chg->dynamic_cv_flag = 1;

	if (chg->dynamic_cv_flag) {
		rc = of_property_read_u32(node, "qcom,second-iterm-ma", &temp);
		if (rc < 0)
			chg->second_iterm_ma = CHANGE_CUTOFF_CURRENT;
		else
			chg->second_iterm_ma = temp;

		rc = of_property_read_u32(node, "qcom,second-fv-max", &temp);
		if (rc < 0)
			chg->second_fv_max = CHANGE_FV_MAX_VOLTAGE;
		else
			chg->second_fv_max = temp;

		rc = of_property_read_u32(node, "qcom,cv-change-current", &temp);
		if (rc < 0)
			chg->cv_change_current = CHANGE_CV_BATT_CURRENT;
		else
			chg->cv_change_current = temp;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void smb5_parse_dt_otg_channel(struct smb5 *chip, struct device_node *node)
{
	int rc;
	struct smb_charger *chg = &chip->chg;

	rc = of_property_read_u32(node,
		"otg_channel", &chg->otg_channel);
	if (rc < 0)
		chg->otg_channel = 0;
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	smb5_parse_dt_otg_channel(chip, node);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static int reverse_usb_type(int val)
{
	int type;
	...
	case CHARGER_TYPE_NON_STANDARD:
		type = POWER_SUPPLY_TYPE_USB_FLOAT;
		break;
	case CHARGER_TYPE_WIRELESS:
		type = POWER_SUPPLY_TYPE_WIRELESS;
		break;
	case CHARGER_TYPE_FCP:
		type = POWER_SUPPLY_TYPE_FCP;
		break;
	default:
		type = POWER_SUPPLY_TYPE_UNKNOWN;
		break;
...
static void wlrx_set_otg(struct smb_charger *chg, enum power_supply_type charger_type)
{
	int ret;
	u8 val = 0;
	...
	if ((chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS) &&
		...
		pr_info("%s: Set typec try.SRC mode when wireless charge start\n", __func__);
		/* Set 0x1544 0x1550 to ensure otg device can be detected in wireless charging */
		ret = smblib_read(chg, TYPE_C_EXIT_STATE_CFG_REG, &val);
		chg->bypass_vsafe_bit = val & BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT;
		ret += smblib_read(chg, TYPE_C_MODE_CFG_REG, &val);
		chg->try_sink_bit = val & EN_TRY_SNK_BIT;
		...
		ret += smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, EN_TRY_SNK_BIT, 0);
		if (ret)
			pr_err("%s typec register write failed\n", __func__);
	} else if ((chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) &&
		(charger_type != POWER_SUPPLY_TYPE_WIRELESS)) {
		pr_info("%s: Reset tpyec try.SRC mode when wireless charge stop\n", __func__);
		ret = smblib_masked_write(chg, TYPE_C_EXIT_STATE_CFG_REG,
			BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT, chg->bypass_vsafe_bit);
		ret += smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
			EN_TRY_SNK_BIT, chg->try_sink_bit);
		if (ret)
			pr_err("%s typec register write failed\n", __func__);
	}
}

static void smb5_set_real_type_prop(struct smb_charger *chg, int charger_type)
{
	enum power_supply_type real_charger_type;

	real_charger_type = reverse_usb_type(charger_type);
	pr_err("%s psy_type: %d-->%d\n", __func__,
		chg->real_charger_type, real_charger_type);

	if ((chg->real_charger_type == real_charger_type) &&
		(real_charger_type != POWER_SUPPLY_TYPE_UNKNOWN)) {
		pr_err("%s the same real_charger_type\n", __func__);
		return;
	}

	wlrx_set_otg(chg, real_charger_type);
	if ((charger_type != CHARGER_TYPE_WIRELESS) &&
		(charger_type != CHARGER_REMOVED)) {
		wireless_charge_wired_vbus_connect_handler();
		if (!chg->pd_active && charger_type != CHARGER_TYPE_FCP)
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	case POWER_SUPPLY_PROP_REAL_TYPE:
		smb5_set_real_type_prop(chg, val->intval);
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		if (chg->usb_third_bc_12)
			chg->ac_online = val->intval;
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pr_info("POWER_SUPPLY_PROP_FLASH_ACTIVE intval=%d, flash=%d\n",
			val->intval, chg->flash_active);
#endif
		if ((chg->chg_param.smb_version == PMI632_SUBTYPE)
			...
			if (rc < 0)
				pr_err("Failed to get USB preset status rc=%d\n",
						rc);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			pr_info("USB preset status rc=%d\n", pval.intval);
#endif
			if (pval.intval) {
...
#else
		rc = hw_get_prop_batt_health(chg, val);
#endif
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_IIN_THERMAL:
		if (val->intval > 0) {
			vote(chg->usb_icl_votable, HW_IIN_THERMAL, true,
				val->intval * THOUSAND);
			chg->iin_thermal = val->intval;
		} else {
			vote(chg->usb_icl_votable, HW_IIN_THERMAL, false, 0);
			chg->iin_thermal = 0;
		}
		break;
	case POWER_SUPPLY_PROP_HIZ_MODE:
		chg->usb_icl_change_irq_enabled = !!val->intval;
		rc = smblib_set_prop_input_suspend(chg, val);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		pr_info("update user ICL to %d\n", val->intval);
		vote(chg->usb_icl_votable, USER_PSY_VOTER, true, val->intval * THOUSAND);
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	default:
		rc = -EINVAL;
...
static const struct power_supply_desc batt_psy_desc = {
	...
	defined(CONFIG_HUAWEI_MIXED_BATT)
	.name = "bk_battery",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
#else
	.name = "battery",
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) {
		chg->try_sink_bit = val;
		val = 0;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
		EN_TRY_SNK_BIT | EN_SNK_ONLY_BIT, val);
	if (rc < 0) {
		dev_err(chg->dev,
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_DEBUG_ACCESS_SINK_REG,
		TYPE_C_DEBUG_ACCESS_MODE_MASK, TYPE_C_DEBUG_CABLE_CHARGING_ENABLE);
	if (rc < 0) {
		pr_err("Couldn't configue TYPE_C_DEBUG config rc=%d\n", rc);
		return rc;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	...
	if (rc < 0) {
		dev_err(chg->dev,
			"Couldn't configure BAT_THERM pull-up rc=%d\n",
			rc);
		return rc;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void smb5_init_status(struct smb_irq_data irq_data)
{
...
extern bool get_fsa_init_status(void);
static void bc12_change_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
		struct smb_charger, bc12_change_work.work);
	struct smb5 *chip = NULL;
	struct smb_irq_data irq_data = {0};
	bool flag = false;
	...
	chip = container_of(chg, struct smb5, chg);
	if (!chip) {
	...
	irq_data.name = "determine-initial-status";
	flag = get_fsa_init_status();
	if (!flag) {
		schedule_delayed_work(&chg->bc12_change_work, msecs_to_jiffies(500)); /* delay 500ms */
	} else {
		smb5_init_status(irq_data);
		/* reset sw_icl_max */
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS)
...
static int is_cable_for_direct_charge(void)
{
	...
	rc = smblib_read(g_chg, TYPE_C_SNK_STATUS_REG, &stat);
	if (rc < 0) {
		pr_err("Couldn't read TYPE_C_STATUS_1 rc=%d\n", rc);
		return -1;
	}
...
static int smb5_determine_initial_status(struct smb5 *chip)
{
	...
	if (!chg->usb_third_bc_12)
		...
		schedule_delayed_work(&chg->bc12_change_work, msecs_to_jiffies(1200)); /* delay 1200ms */
	return 0;
}
#else
/****************************
...
static int smb5_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	INIT_DELAYED_WORK(&chg->bc12_change_work, bc12_change_work);
	rc = cc_check_ops_register(&cc_check_ops);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	...
	g_chg = chg;
	smblib_masked_write(chg, USBIN_ADAPTER_ALLOW_CFG_REG,
		USBIN_ADAPTER_ALLOW_MASK, USBIN_ADAPTER_ALLOW_5V_TO_9V);

	/* set cable detect time to 9ms */
	rc = smblib_write(chg, TYPE_C_LEGACY_CABLE_CFG_REG, LEGACY_CABLE_DET_WINDOW_9MS);
	if (rc < 0) {
		pr_err("Couldn't configure typec CABLE_DETECT rc=%d\n", rc);
		return rc;
	}
	pr_info("set TYPEC_CABLE_DETECT_TIME %d\n", LEGACY_CABLE_DET_WINDOW_9MS);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
