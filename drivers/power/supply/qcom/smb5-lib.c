...
static const struct apsd_result smblib_apsd_results[] = {
	...
	[WIRELESS] = {
		.name   = "WIRELESS",
		.bit    = 0,
		.pst    = POWER_SUPPLY_TYPE_WIRELESS,
	},
	[FCP] = {
		.name = "FCP",
		.bit = 0,
		.pst = POWER_SUPPLY_TYPE_FCP,
	},
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
};
...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if (chg->usb_third_bc_12) {
		for (i = 0; i < ARRAY_SIZE(smblib_apsd_results); i++) {
			if (smblib_apsd_results[i].pst != chg->real_charger_type)
				continue;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->usb_third_bc_12)
		enable = false; /* force pmi632 not do bc12 */
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("%s: pd_active %d charge_type %d apsd %d\n", __func__,
		chg->pd_active, chg->real_charger_type, apsd_result->pst);

	if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS)
		return &smblib_apsd_results[WIRELESS];
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static bool smblib_usb_vbus_rising_flag(struct smb_charger *chg)
{
	int rc;
	u8 stat = 0;
			...
			int chg_disable, const char *client)
{
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	union power_supply_propval pval = {0, };
	static bool vbus_rising_flag = false;
	...
	pr_info("%s: %s charging\n", __func__, chg_disable ? "disable" : "enable");
	vbus_rising_flag = smblib_usb_vbus_rising_flag(chg);
	if (pval.intval) {
		if (chg_disable) {
			if (vbus_rising_flag)
				power_event_bnc_notify(POWER_BNT_THIRDPLAT_CHARGING,
					POWER_NE_THIRDPLAT_CHARGING_ERROR, NULL);
			else
				power_event_bnc_notify(POWER_BNT_THIRDPLAT_CHARGING,
					POWER_NE_THIRDPLAT_CHARGING_STOP, NULL);
		} else {
			power_event_bnc_notify(POWER_BNT_THIRDPLAT_CHARGING,
				POWER_NE_THIRDPLAT_CHARGING_START, NULL);
		}
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->otg_channel) {
		return vbus_ch_open(VBUS_CH_USER_WIRED_OTG,
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->otg_channel) {
		rc = vbus_ch_close(VBUS_CH_USER_WIRED_OTG,
			VBUS_CH_TYPE_BOOST_GPIO, true, false);
		if (rc < 0) {
			smblib_err(chg, "vbus_ch_close fail rc=%d\n", rc);
			return rc;
		}
				...
				union power_supply_propval *val)
{
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	chg_enable = get_effective_result(chg->chg_disable_votable);
	if (chg_enable < 0) /* no votes */
		chg_enable = 1;
	else
		chg_enable = !chg_enable;
	...
	if (((direct_charge_get_stage_status() <= DC_STAGE_CHARGE_DONE) &&
		(direct_charge_get_stage_status() >= DC_STAGE_ADAPTER_DETECT)) ||
		(chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) ||
		(pval.intval && !chg->usb_icl_change_irq_enabled && !chg->input_suspend))
		val->intval = chg_enable ? POWER_SUPPLY_STATUS_CHARGING :
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
int hw_get_prop_batt_health(struct smb_charger *chg,
	union power_supply_propval *val)
{
	union power_supply_propval pval;
	int rc, batt_temp;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (!(bool)val->intval) {
		rc = smblib_write(chg, USBIN_CONT_AICL_THRESHOLD_REG, 0);
		if (rc < 0) {
			smblib_err(chg, "Couldn't set USBIN_CONT_AICL_THRESHOLD_REG to 0, rc=%d\n",
				rc);
			return rc;
		}
	}
			       ...
			       union power_supply_propval *val)
{
	...
	if (chg->flash_active)
		...
		val->intval = (stat & USE_USBIN_BIT) && (stat &
			VALID_INPUT_POWER_SOURCE_STS_BIT);
#else
	val->intval = (stat & USE_USBIN_BIT) &&
		      (stat & VALID_INPUT_POWER_SOURCE_STS_BIT);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pd_dpm_set_cc_orientation(val->intval - 1);
#endif  /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
					...
					int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		...
		/* if usb_current is less then 500mA */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) {
		chg->try_sink_bit = power_role & EN_TRY_SNK_BIT;
...
irqreturn_t chg_state_change_irq_handler(int irq, void *data)
{
	...
	if (pval.intval && !((direct_charge_get_stage_status() <= DC_STAGE_CHARGE_DONE) &&
		(direct_charge_get_stage_status() >= DC_STAGE_ADAPTER_DETECT))) {
		switch (stat) {
		case TRICKLE_CHARGE:
		case PRE_CHARGE:
	...
	if (stat == TERMINATE_CHARGE) {
		release_charge_lock();
		smblib_err(chg, "TERMINATE_CHARGE IRQ: %s\n", irq_data->name);
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void power_charger_event_process(struct smb_charger *chg, bool vbus_rising)
{
	static enum power_supply_type last_type;
	enum power_supply_type current_type = chg->real_charger_type;

	if (vbus_rising) {
		if (current_type == POWER_SUPPLY_TYPE_WIRELESS)
		...
		if (last_type == POWER_SUPPLY_TYPE_WIRELESS)
	...
	if (vbus_rising) {
		...
		if (chg->fcc_stepper_enable)
			vote(chg->fcc_votable, FCC_STEPPER_VOTER,
							true, 1500000);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pr_info("hard_reset lock chg type det false\n");
		if (chg->usb_third_bc_12 &&
			(chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS))
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	smblib_dbg(chg, PR_INTERRUPT, "set CHG_PLUGIN\n");
	power_supply_set_int_property_value("huawei_charge",
		POWER_SUPPLY_PROP_CHG_PLUGIN, vbus_rising);
	power_charger_event_process(chg, vbus_rising);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("vbus_rising=%d, ignore=%d\n", vbus_rising, chg->ignore_vbus_irq);
	if (chg->ignore_vbus_irq)
		return;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	...
	if (vbus_rising) {
		...
		if (chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS)
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
			(void)smblib_request_dpdm(chg, true);
		...
		power_event_bnc_notify(POWER_BNT_THIRDPLAT_CHARGING,
			POWER_NE_THIRDPLAT_CHARGING_STOP, NULL);
		power_event_bnc_notify(POWER_BNT_THIRDPLAT_PD,
			POWER_NE_THIRDPLAT_PD_STOP, NULL);
		pd_dpm_set_ctc_cable_flag(false);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	}
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	typec_mode = smblib_get_prop_typec_mode(chg);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("%s typec_mode=%d usb_third_bc_12=%d pst=%d\n",
		__func__, typec_mode, chg->usb_third_bc_12, pst);
	if (typec_rp_med_high(chg, typec_mode) && !chg->usb_third_bc_12) {
	...
	case POWER_SUPPLY_TYPE_FCP:
		pr_info("set fcp current\n");
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, FCP_CURRENT_UA);
		break;
	case POWER_SUPPLY_TYPE_WIRELESS:
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, WIRELESS_CURRENT_UA);
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	power_supply_set_int_property_value("huawei_charge",
		POWER_SUPPLY_PROP_CHG_TYPE, apsd_result->pst);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
void non_pmi_handle_apsd_done(struct smb_charger *chg)
{
	if (!chg) {
	...
	pr_info("%s %d detected\n", __func__, chg->real_charger_type);
	power_supply_set_int_property_value("huawei_charge",
		POWER_SUPPLY_PROP_CHG_TYPE, chg->real_charger_type);
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_FCP)
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	cancel_delayed_work_sync(&chg->period_update_work);
	schedule_delayed_work(&chg->period_update_work,
		round_jiffies_relative(msecs_to_jiffies(PERIOD_DEFAULT_MS)));
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("typec_src_removal dis vote PD_SUSPEND_SUPPORTED_VOTER\n");
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static void wlrx_sw_debounce_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
		struct smb_charger, wlrx_sw_work.work);

	chg->ignore_vbus_irq = false;
	if (chg->sink_src_mode != SINK_MODE)
		smblib_usb_plugin_locked(chg);
}

static int usbin_set_suspend_prop(struct smb_charger *chg, bool val)
{
	/* 1: suspend when UVLO happens, 0: not suspend when UVLO happens */
	return smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG,
		SUSPEND_ON_COLLAPSE_USBIN_BIT,
		val ? 0 : SUSPEND_ON_COLLAPSE_USBIN_BIT);
}

static int wireless_notifier_call(struct notifier_block *wireless_nb,
	unsigned long event, void *data)
{
	struct smb_charger *chg = container_of(wireless_nb,
		struct smb_charger, wireless_nb);
	...
	case POWER_NE_WLC_CHARGER_VBUS:
		(void)usbin_set_suspend_prop(chg, true);
		break;
	default:
		break;
	}
...
static int vbus_notifier_call(struct notifier_block *vbus_nb,
	unsigned long event, void *data)
{
	struct smb_charger *chg = container_of(vbus_nb,
		struct smb_charger, vbus_nb);
	...
	case POWER_NE_WIRELESS_DISCONNECT:
		(void)usbin_set_suspend_prop(chg, false);
		break;
	default:
		break;
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			pr_info("SINK_MODE, real_charger_type=%d\n", chg->real_charger_type);
			if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) {
				chg->ignore_vbus_irq = true;
				usbswitch_common_chg_type_det(true);
				/* delay 500ms for cc debounce */
				schedule_delayed_work(&chg->wlrx_sw_work, msecs_to_jiffies(500));
			}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pr_info("typec detach, real_charger_type=%d\n", chg->real_charger_type);
		if (chg->real_charger_type != POWER_SUPPLY_TYPE_WIRELESS)
			chg->real_charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
				...
				const union power_supply_propval *val)
{
	...
	reg_val = val->intval ? BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT : 0;
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_WIRELESS) {
		chg->bypass_vsafe_bit = reg_val;
		reg_val = BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT;
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	rc = smblib_masked_write(chg, TYPE_C_EXIT_STATE_CFG_REG,
		BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT, reg_val);
	if (rc < 0)
...
#define HLTHERM_RUNTEST_MAX_VOL         4100000
...
static void jeita_update_work(struct work_struct *work)
{
	...
	jeita_hard_thresholds[HLTHERM_RUNTEST_COLD] = 0x4f25;
	jeita_hard_thresholds[HLTHERM_RUNTEST_HOT] = 0x5f5;
#endif
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#ifdef CONFIG_HLTHERM_RUNTEST
	tmp[0] = HLTHERM_RUNTEST_MAX_VOL;
	tmp[1] = HLTHERM_RUNTEST_MAX_VOL;
#endif /* CONFIG_HLTHERM_RUNTEST */
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define FIX_PMI632_DISCHARGE_DELAY 100
#define PEROID_UPDATE_MS           5000
#define PEROID_UPDATE_DEFAULT_MS   1000
#define ARB_VBUS_UV_THRESHOLD      4200000
#define ARB_VBUS_UV_MIN_THRESHOLD  3800000
#define ARB_IBUS_UA_THRESHOLD      100000
static void update_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work,
		struct smb_charger, period_update_work.work);
	int rc, vol_now, cur_now, ibus, chg_enable, raw_hi_thresh;
	union power_supply_propval val = { 0, };
	static bool cv_change_flag = true;
	static int cv_change_count;
	u8 *buf;
	...
	pr_info("cur_now = %d, vol_now = %d, ibus = %d\n", cur_now, vol_now, ibus);

	if ((vol_now < ARB_VBUS_UV_THRESHOLD) && (ibus < ARB_IBUS_UA_THRESHOLD) &&
		(vol_now > ARB_VBUS_UV_MIN_THRESHOLD) && (ibus > 0) &&
		!chg->usb_icl_change_irq_enabled && !chg->input_suspend && chg_enable) {
		rc = vote(chg->usb_icl_votable, USER_VOTER, true, 0);
		if (rc < 0)
			pr_err("can't vote usb to suspend, usb rc = %d\n", rc);
		mdelay(FIX_PMI632_DISCHARGE_DELAY);
		rc = vote(chg->usb_icl_votable, USER_VOTER, false, 0);
		if (rc < 0)
			pr_err("can't vote usb not suspend, usb rc = %d\n", rc);
	}

	if (chg->dynamic_cv_flag) {
		if (cv_change_flag && (cur_now < chg->cv_change_current) && (cur_now > 0))
			cv_change_count++;
		else
			cv_change_count = 0;

		if (cv_change_count > CHANGE_CV_TIMES) {
			raw_hi_thresh = RAW_ITERM(chg->second_iterm_ma, ITERM_LIMITS_PMI632_MA);
			raw_hi_thresh = sign_extend32(raw_hi_thresh, FIFTY_FOR_ITERM_CURRENT);
			buf = (u8 *)&raw_hi_thresh;
			raw_hi_thresh = buf[1] | (buf[0] << EIGHT_BIT);
			rc = smblib_batch_write(chg, CHGR_ADC_ITERM_UP_THD_MSB_REG,
				(u8 *)&raw_hi_thresh, TWO_BIT);
			if (rc < 0) {
				dev_err(chg->dev,
					"Couldn't configure ITERM shold HIGH rc=%d\n", rc);
				return;
			}
			vote(chg->fv_votable, BATT_PROFILE_VOTER, true, chg->second_fv_max);
			cv_change_count = 0;
			cv_change_flag = false;
		}
	}

	schedule_delayed_work(&chg->period_update_work,
		round_jiffies_relative(msecs_to_jiffies(PEROID_UPDATE_MS)));
}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	INIT_DELAYED_WORK(&chg->period_update_work, update_work);
	schedule_delayed_work(&chg->period_update_work,
		round_jiffies_relative(msecs_to_jiffies(PEROID_UPDATE_DEFAULT_MS)));
	INIT_DELAYED_WORK(&chg->wlrx_sw_work, wlrx_sw_debounce_work);

	chg->wireless_nb.notifier_call = wireless_notifier_call;
	rc = power_event_bnc_register(POWER_BNT_WLC, &chg->wireless_nb);
	if (rc < 0)
		smblib_err(chg, "register wireless_notifier failed, rc=%d\n", rc);
	chg->vbus_nb.notifier_call = vbus_notifier_call;
	rc = power_event_bnc_register(POWER_BNT_CONNECT, &chg->vbus_nb);
	if (rc < 0)
		smblib_err(chg, "register vbus_notifier failed, rc=%d\n", rc);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
