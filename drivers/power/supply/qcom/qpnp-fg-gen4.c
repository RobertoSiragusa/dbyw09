...
#define VOL_M_TO_U  1000  /* mV convert to uV */
...
static int fg_gen4_get_battery_temp(struct fg_dev *fg, int *val)
{
	...
	if (fg->battery_missing) {
		pr_debug("Battery Missing temp = 250\n");
		*val = 250;
		return 0;
	}

	if (chip->batt_therm_chan) {
		rc = iio_read_channel_processed(chip->batt_therm_chan, val);
		if (rc < 0) {
	...
	if (is_input_present(fg)) {
		rc = fg_get_battery_current(fg, &ibat);
		if (rc < 0)
			return rc;
		ibat *= (-1);
		tmp = *val;

		for (i = 0; i < ARRAY_EIGHTEEN - 1;) {
			if (ibat > (chip->fg_temp_ir[i] * 1000)) {
				tmp += chip->fg_temp_ir[i + 1];
				break;
			}
			i += 2; /* single count is current */
		}
		*val = tmp;
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	fg->bp.full_delta_uv = fg->bp.float_volt_uv - fg->bp.vbatt_full_mv * VOL_M_TO_U;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define PMIC_FG_RESET_ADDRESS 0x40ba
#define PMIC_FG_RESET_VALUE_OFF 0x04
#define PMIC_FG_RESET_VALUE_ON 0
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	val = PMIC_FG_RESET_VALUE_OFF;
	rc = fg_write(fg, PMIC_FG_RESET_ADDRESS, &val, 1);
	if (rc < 0)
		pr_err("fg reset close fail rc=%d\n", rc);
	msleep(10);

	val = PMIC_FG_RESET_VALUE_ON;
	rc = fg_write(fg, PMIC_FG_RESET_ADDRESS, &val, 1);
	if (rc < 0)
		pr_err("fg reset open fail rc=%d\n", rc);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("Enter FVSS mode, SOC=%d slope=%d timer=%d\n", soc,
...
#else
	pr_info("Exit FVSS mode, work_status=%d\n",
		work_busy(&chip->soc_scale_work));
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pr_info("fg_gen4_set_vbatt_low set\n");
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
		chip->vbatt_low = true;
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("vbatt_low irq %d triggered vbatt_mv: %d msoc_raw:%d\n", irq,
		vbatt_mv, msoc_raw);
#else
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("Calculated SOC=%d SOC reported=%d timer resolution=%d\n",
				       ...
				       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		pval->intval = fg->bp.vbatt_full_mv * VOL_M_TO_U + fg->bp.full_delta_uv;
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	default:
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		...
		break;
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	case POWER_SUPPLY_PROP_SYNC_FILTER:
		break;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		fg->bp.vbatt_full_mv = (pval->intval - fg->bp.full_delta_uv) / VOL_M_TO_U;
		if (fg->bp.vbatt_full_mv > 0) {
			rc = fg_set_constant_chg_voltage(fg,
				fg->bp.vbatt_full_mv * VOL_M_TO_U);
			if (rc < 0)
				return rc;
		}
		break;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define HIGH_CURRENT_9              7500
#define HIGH_CURRENT_8              6500
#define HIGH_CURRENT_7              5500
#define HIGH_CURRENT_6              4500
#define HIGH_CURRENT_5              3500
#define HIGH_CURRENT_4              2500
#define HIGH_CURRENT_3              1500
#define HIGH_CURRENT_2              500
#define HIGH_CURRENT_1              0
#define CURRENT_TEMP_IR_9           85
#define CURRENT_TEMP_IR_8           75
#define CURRENT_TEMP_IR_7           65
#define CURRENT_TEMP_IR_6           55
#define CURRENT_TEMP_IR_5           50
#define CURRENT_TEMP_IR_4           40
#define CURRENT_TEMP_IR_3           30
#define CURRENT_TEMP_IR_2           20
#define CURRENT_TEMP_IR_1           10
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	rc = of_property_match_string(fg->dev->of_node, "io-channel-names", "batt_therm");
	if (rc >= 0) {
		chip->batt_therm_chan = devm_iio_channel_get(fg->dev, "batt_therm");
		if (IS_ERR(chip->batt_therm_chan)) {
			rc = PTR_ERR(chip->batt_therm_chan);
			if (rc != -EPROBE_DEFER)
				pr_err("Couldn't get batt_therm_chan rc=%d\n", rc);
			chip->batt_id_chan = NULL;
			return rc;
		}
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	pr_info("cuoff %d, empty %d, scale %d\n",
		chip->dt.cutoff_volt_mv,
		chip->dt.empty_volt_mv,
		chip->dt.vbatt_scale_thr_mv);
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	rc = of_property_read_u32_array(node, "qcom,fg-temp-ir-for-current", tmp, ARRAY_EIGHTEEN);
	if (rc) {
		pr_err("Cound't parse temp ir use default value\n");
		chip->fg_temp_ir[ZERO] = HIGH_CURRENT_9;
		chip->fg_temp_ir[ONE] = CURRENT_TEMP_IR_9;
		chip->fg_temp_ir[TWO] = HIGH_CURRENT_8;
		chip->fg_temp_ir[THREE] = CURRENT_TEMP_IR_8;
		chip->fg_temp_ir[FOUR] = HIGH_CURRENT_7;
		chip->fg_temp_ir[FIVE] = CURRENT_TEMP_IR_7;
		chip->fg_temp_ir[SIX] = HIGH_CURRENT_6;
		chip->fg_temp_ir[SEVEN] = CURRENT_TEMP_IR_6;
		chip->fg_temp_ir[EIGHT] = HIGH_CURRENT_5;
		chip->fg_temp_ir[NINE] = CURRENT_TEMP_IR_5;
		chip->fg_temp_ir[TEN] = HIGH_CURRENT_4;
		chip->fg_temp_ir[ELEVEN] = CURRENT_TEMP_IR_4;
		chip->fg_temp_ir[TWELVE] = HIGH_CURRENT_3;
		chip->fg_temp_ir[THIRTEEN] = CURRENT_TEMP_IR_3;
		chip->fg_temp_ir[FOURTEEN] = HIGH_CURRENT_2;
		chip->fg_temp_ir[FIFTEEN] = CURRENT_TEMP_IR_2;
		chip->fg_temp_ir[SIXTEEN] = HIGH_CURRENT_1;
		chip->fg_temp_ir[SEVENTEEN] = CURRENT_TEMP_IR_1;
	} else {
		for (i = 0; i < ARRAY_EIGHTEEN; i++)
			chip->fg_temp_ir[i] = tmp[i];
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
	return 0;
}
