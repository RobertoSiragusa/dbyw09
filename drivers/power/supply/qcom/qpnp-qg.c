...
struct qpnp_qg *g_qpnp_qg_dev = NULL;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define DEFAULT_SHUTDOWN_SOC_SECS	36000
#else
#define DEFAULT_SHUTDOWN_SOC_SECS	360
#endif
#define DEFAULT_COLD_TEMP_THRESHOLD	0
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define DEFAULT_SHUTDOWN_TEMP_DIFF	600	/* 60 degC */
#else
...
#define NORMAL_CURRENT              500
#define HIGH_CURRENT_TEMP_IR        50
#define NORMAL_CURRENT_TEMP_IR      30
#define LOW_CURRENT_TEMP_IR         20
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	rc = of_property_read_u32_array(node, "qcom,qg-temp-ir-for-current", tmp, ARRAY_SIX);
	if (rc) {
		pr_err("Cound't parse temp ir use default value\n");
		chip->qg_temp_ir[ZERO] = HIGH_CURRENT;
		chip->qg_temp_ir[ONE] = HIGH_CURRENT_TEMP_IR;
		chip->qg_temp_ir[TWO] = NORMAL_CURRENT;
		chip->qg_temp_ir[THREE] = NORMAL_CURRENT_TEMP_IR;
		chip->qg_temp_ir[FOUR] = 0;
		chip->qg_temp_ir[FIVE] = LOW_CURRENT_TEMP_IR;
	} else {
		for (i = 0; i < ARRAY_SIX; i++)
			chip->qg_temp_ir[i] = tmp[i];
	}
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
int get_batt_id_uv(void)
{
	int rc;
	int batt_id_mv = 0;

	if (g_qpnp_qg_dev == NULL)
		return 400000;

	/* Read battery-id */
	rc = iio_read_channel_processed(g_qpnp_qg_dev->batt_id_chan, &batt_id_mv);
	if (rc < 0) {
		pr_err("Failed to read BATT_ID over ADC, rc=%d\n", rc);
		return rc;
	}
