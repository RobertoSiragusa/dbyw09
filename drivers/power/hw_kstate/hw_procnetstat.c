...
static int make_hash_code(const char *comm, uid_t uid, uint32_t *hash_code)
{
	...
	snprintf(key, (KEY_LENGTH - 1), "%s_%d", comm, uid);
...
static void init_default_shared_uids(void) {
	memset(shared_uids, 0, sizeof(int) * MAX_SHARED_UID_NUM);
	shared_uids[0] = 0;
...
static void init_thread_decompose_uids(void) {
	memset(thread_decompose_uids, 0, sizeof(int) * MAX_THREAD_DECOMPOSE_UID_NUM);
