...
static void fsa9685_resume_irq(void)
{
	...
	ret = switch_write_reg_mask(di->client, FSA9685_REG_CONTROL2, 0, FSA9685_DCD_TIME_OUT_MASK);
...
static void rt8979_intb_work(struct work_struct *work)
{
	...
	if (muic_status1 & RT8979_DCDT) {
		...
		if (!rt8979_dcd_timeout_enabled) {
			...
			switch_write_reg(di->client, FSA9685_REG_INTERRUPT_MASK, 0);
			switch_write_reg_mask(di->client, RT8979_REG_USBCHGEN, 0,
				RT8979_REG_USBCHGEN_ACCPDET_STAGE1);
			switch_write_reg_mask(di->client, RT8979_REG_TIMING_SET_2, 0x38, 0x38);
			switch_write_reg_mask(di->client, RT8979_REG_USBCHGEN,
				RT8979_REG_USBCHGEN_ACCPDET_STAGE1,
				RT8979_REG_USBCHGEN_ACCPDET_STAGE1);
			redo_bc12 = true;
	...
	if (!is_support_fcp() &&
		((RT8979_REG_ALLMASK != switch_read_reg(di->client, FSA9685_REG_ACCP_INTERRUPT_MASK1)) ||
		(RT8979_REG_ALLMASK != switch_read_reg(di->client, FSA9685_REG_ACCP_INTERRUPT_MASK2)))) {
		hwlog_info("disable fcp interrrupt again!!\n");
		ret2 |= switch_write_reg_mask(di->client, FSA9685_REG_CONTROL2, 0, FSA9685_ACCP_OSC_ENABLE);
		if (!is_rt8979())
	...
	OUT:
	if ((di->pd_support == 0) && (usb_switch_wakelock_flag == USB_SWITCH_NEED_WAKE_UNLOCK) &&
		(invalid_times == 0))
...
static void fsa9685_intb_work(struct work_struct *work)
{
	...
	if (!is_support_fcp() &&
		...
		ret2 |= switch_write_reg_mask(di->client, FSA9685_REG_CONTROL2,
			di->dcd_timeout_force_enable, FSA9685_DCD_TIME_OUT_MASK);
		ret2 |= switch_write_reg(di->client, FSA9685_REG_ACCP_INTERRUPT_MASK1, 0xFF);
		ret2 |= switch_write_reg(di->client, FSA9685_REG_ACCP_INTERRUPT_MASK2, 0xFF);
		hwlog_info("%s : read ACCP interrupt,reg[0x59]=0x%x,reg[0x5A]=0x%x\n", __func__,
			switch_read_reg(di->client, FSA9685_REG_ACCP_INTERRUPT1),
			switch_read_reg(di->client, FSA9685_REG_ACCP_INTERRUPT2));
		if (ret2 < 0)
...
void fcp_protocol_restart(void)
{
	...
	/* disable accp interrupt */
	ret |= switch_write_reg_mask(di->client, FSA9685_REG_CONTROL2, FSA9685_ACCP_OSC_ENABLE,
...
static int rt8979_suspend_noirq(struct fsa9685_device_info *chip)
{
	hwlog_info("%s\n", __func__);
	if (chip->irq_waiting) {
...
static int fsa9685_suspend_noirq(struct device *dev)
{
	struct fsa9685_device_info *di = dev_get_drvdata(dev);
	...
	if (is_rt8979())
		return rt8979_suspend_noirq(di);
...
static int rt8979_resume(struct fsa9685_device_info *chip)
{
	...
	mutex_unlock(&chip->irq_lock);
	if (chip->irq_waiting) {
		rt8979_deal_resume_irq(chip->irq, chip);
		enable_irq(chip->irq);
	}
	return 0;
}
...
static const struct dev_pm_ops fsa9685_pm_ops = {
	...
	.freeze = fsa9685_suspend,
	.thaw = fsa9685_resume,
	.poweroff = fsa9685_suspend,
	.restore = fsa9685_resume,
	.suspend_noirq = fsa9685_suspend_noirq,
#endif
};
