...
/* rt8979 ustd retried count */
#define RT8979_NSTD_RETRY_CNT       5
...
static inline int rt8979_toggle_bc12(struct fsa9685_device_info *chip)
{
	...
	while (retry_times) {
		...
		if (ret >= 0)
			break;
		retry_times--;
		usleep_range(50000, 51000); /* 50000,51000: delay 50ms */
	}
	if (retry_times == 0) {
	...
	usleep_range(35000, 36000); /* 35000,36000: delay 35ms for bc12 operate */
out:
...
static int rt8979_bc12_post_process(struct fsa9685_device_info *chip)
{
	...
	case RT8979_USB_STATUS_SDP:
		...
		if (chip->sdp_retried < SDP_RETRY_COUNT) {
			chip->sdp_retried++;
			rt8979_enable_bc12(chip, true);
			return 0;
		}
	...
	if (chip->chg_type == CHARGER_TYPE_NON_STANDARD) {
		hwlog_info("%s nstd %u\n", __func__, chip->nstd_retried);
		if (chip->nstd_retried < RT8979_NSTD_RETRY_CNT) {
			inform_psy = !chip->nstd_retried;
			chip->nstd_retried++;
			rt8979_set_dcd_timeout(chip, DCD_TIMEOUT);
			rt8979_enable_bc12(chip, true);
...
static int rt8979_vbus_change_irq_handler(struct fsa9685_device_info *chip)
{
	...
	if (!atomic_read(&chip->chg_type_det) &&
		(power_platform_get_charger_type() == CHARGER_TYPE_WIRELESS))
		return 0;
