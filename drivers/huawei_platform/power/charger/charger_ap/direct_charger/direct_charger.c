...
static int direct_charge_redetect_adapter_by_voltage(unsigned int mode)
{
	...
	if (!(mode & (LVC_MODE | SC_MODE | SC4_MODE)))
		return mode;
...
void direct_charge_get_ffc_para(void)
{
	...
	vbat_main = dcm_get_ic_vbtb_with_comp(CHARGE_IC_MAIN, l_di->cur_mode, l_di->comp_para.vbat_comp);
	vbat_aux = dcm_get_ic_vbtb_with_comp(CHARGE_IC_AUX, l_di->cur_mode, l_di->comp_para.vbat_comp);
...
void cam_stop_charging(void)
{
	...
	if ((scp_adp_mode & ADAPTER_SUPPORT_SCP_B_SC) != ADAPTER_SUPPORT_SCP_B_SC)
		return;
