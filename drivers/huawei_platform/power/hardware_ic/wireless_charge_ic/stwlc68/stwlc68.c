...
exit:
	...
	power_wakeup_unlock(g_stwlc68_wakelock, false);
}
...
static irqreturn_t stwlc68_interrupt(int irq, void *_di)
{
	...
	power_wakeup_lock(g_stwlc68_wakelock, false);
	hwlog_info("[%s] ++\n", __func__);
	if (di->irq_active) {
		disable_irq_nosync(di->irq_int);
		di->irq_active = 0;
...
static void stwlc68_shutdown(struct i2c_client *client)
{
	...
	if (wlrx_get_wireless_channel_state() == WIRELESS_CHANNEL_ON) {
		...
		(void)stwlc68_set_vfc(ADAPTER_5V * POWER_MV_PER_V, di);
		(void)stwlc68_set_rx_vout(ADAPTER_5V * POWER_MV_PER_V, di);
...
static int stwlc68_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_stwlc68_wakelock = power_wakeup_source_register(di->dev, "stwlc68_wakelock");
...
ops_regist_fail:
	power_wakeup_source_unregister(g_stwlc68_wakelock);
