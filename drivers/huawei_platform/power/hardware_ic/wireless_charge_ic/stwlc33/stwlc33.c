...
static void stwlc33_irq_work(struct work_struct *work)
{
	...
	if (!di) {
		hwlog_err("%s: di null\n", __func__);
		power_wakeup_unlock(g_stwlc33_wakelock, false);
		return;
	}
...
static irqreturn_t stwlc33_interrupt(int irq, void *_di)
{
	...
	power_wakeup_lock(g_stwlc33_wakelock, false);
	hwlog_info("%s ++\n", __func__);
	if (di->irq_active == 1) {
		disable_irq_nosync(di->irq_int);
		di->irq_active = 0;
...
static int stwlc33_get_chip_id(u16 *chip_id)
{
	int ret;

	ret = stwlc33_read_word(STWLC33_RX_CHIP_ID_ADDR, chip_id);
...
static int stwlc33_dev_check(struct stwlc33_device_info *di)
{
	int ret;
	u16 chip_id = 0;

	wlps_control(WLTRX_IC_AUX, WLPS_TX_PWR_SW, true);
	power_usleep(DT_USLEEP_10MS); /* delay for power on */
	ret = stwlc33_get_chip_id(&chip_id);
	if (ret) {
		hwlog_err("dev_check: failed\n");
		wlps_control(WLTRX_IC_AUX, WLPS_TX_PWR_SW, false);
		return ret;
	}
	wlps_control(WLTRX_IC_AUX, WLPS_TX_PWR_SW, false);
	...
	const struct i2c_device_id *id)
{
	...
	g_stwlc33_wakelock = power_wakeup_source_register(di->dev, "stwlc33_wakelock");
...
stwlc33_fail_3:
	power_wakeup_source_unregister(g_stwlc33_wakelock);
