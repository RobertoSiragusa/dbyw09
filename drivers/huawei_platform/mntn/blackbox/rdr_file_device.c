...
{
	struct rdr_file *file = kzalloc(sizeof(struct rdr_file), GFP_ATOMIC);
	if (file == NULL) {
		PRINT_ERROR("%s(): kzalloc rdr_file header failed", __func__);
		return NULL;
	}
	file->name = name;
	file->size = size;
	file->buf = vmalloc(size);
	if (file->buf == NULL) {
		PRINT_ERROR("%s(): kzalloc file->buf failed, size: %llu", __func__, size);
		kfree(file);
		return NULL;
	}
...
static int rdr_write_buflist_to_file(void)
{
	...
	list_for_each_safe(cur, next, &rdr_file_list) {
		...
		if (file->used_size > file->size) {
			PRINT_ERROR("%s(): used_size(%llx) is larger than allocated size(%llx)",
...
void rdr_wait_file_devices(void)
{
	rdr_wait_partition("/data/");
	rdr_wait_partition("/data/local/tmp");
}
