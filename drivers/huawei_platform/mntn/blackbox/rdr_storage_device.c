...
void rdr_wait_storage_devices(void)
{
	rdr_wait_partition(g_storage_device->sys_path);
}
...
int rdr_register_logsave_info(int modid, const struct rdr_logsave_info *info)
{
...
static int rdr_getlast_logsave_index(int modid)
{
	int i;
	struct rdr_logsave_info *curr_info = NULL;
	for (i = 0; i < g_last_head_info->used_number; i++) {
		curr_info = &g_last_head_info->rdr_info[i];
...
struct rdr_region *rdr_acquire_region(int modid)
{
	int index;
	struct rdr_logsave_info *curr_info = NULL;
	struct rdr_region *region = NULL;

	index = rdr_getlast_logsave_index(modid);
	if (index == -1)
		return NULL;
	curr_info = &g_last_head_info->rdr_info[index];
	if (curr_info->magic != RDR_BOOT_RESPONSE_MAGIC) {
		PRINT_INFO("%s(): mod %d is found, but magic is not set, return now",
			__func__, modid);
		return NULL;
	}
...
struct module_data rdr_storage_device_data = {
	...
	.init_done = rdr_storage_device_release,
