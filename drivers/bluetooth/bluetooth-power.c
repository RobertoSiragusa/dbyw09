...
static int bt_power_populate_dt_pinfo(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		bt_power_pdata->bt_gpio_indep_ctrl =
			of_get_named_gpio(pdev->dev.of_node,
						"qca,bt-indep-ctrl-gpio",  0);
...
static int bt_power_independent_gpio()
{
	...
	rc = gpio_request(bt_indep_ctrl_gpio, "bt_indep_ctrl_n");
	if (rc) {
		BT_PWR_ERR("unable to request independent gpio %d %d\n", bt_indep_ctrl_gpio, rc);
	} else {
		rc = gpio_direction_output(bt_indep_ctrl_gpio, 0);
		if (rc)
			BT_PWR_ERR("Unable to set independent gpio %d ret %d\n", bt_indep_ctrl_gpio, rc);
	}
	BT_PWR_INFO("set up bt independent gpio success");
	return rc;
}
...
static int bt_power_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = bt_power_independent_gpio();
		if (ret < 0) {
			BT_PWR_ERR("Failed to power independent gpio");
			goto free_pdata;
		}
