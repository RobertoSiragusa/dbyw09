...
llcc_ecc_irq_handler(int irq, void *edev_ctl)
{
	...
	for (i = 0; i < drv->num_banks; i++) {
		...
		if (!ret && (drp_error & SB_ECC_ERROR)) {
			...
			report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_SYSCACHE, DMD_CACHE_TYPE_CE);
			ret = dump_syn_reg(edev_ctl, LLCC_DRAM_CE, i);
		} else if (!ret && (drp_error & DB_ECC_ERROR)) {
			edac_printk(KERN_CRIT, EDAC_LLCC,
				    "Double Bit Error detected in Data RAM\n");
			report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_SYSCACHE, DMD_CACHE_TYPE_UE);
