...
static void kryo2xx_silver_parse_cpumerrsr(struct erp_local_data *ed)
{
	...
	if (ed->err == SBE) {
		...
		report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L1, DMD_CACHE_TYPE_CE);
	}
	else if (ed->err == DBE) {
		errors[KRYO2XX_SILVER_L1_UE].func(ed->drv->edev_ctl,
			smp_processor_id(), L1_CACHE,
			errors[KRYO2XX_SILVER_L1_UE].msg);
		report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L1, DMD_CACHE_TYPE_UE);
	}
...
static void kryo2xx_silver_parse_l2merrsr(struct erp_local_data *ed)
{
	...
	if (ed->err == SBE) {
		...
		report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L2, DMD_CACHE_TYPE_CE);
	}
	else if (ed->err == DBE) {
		errors[KRYO2XX_SILVER_L2_UE].func(ed->drv->edev_ctl,
			smp_processor_id(), L2_CACHE,
			errors[KRYO2XX_SILVER_L2_UE].msg);
		report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L2, DMD_CACHE_TYPE_UE);
	}
