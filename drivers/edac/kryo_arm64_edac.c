	...
	case L1:
		if (KRYO_ERRXSTATUS_UE(errxstatus)) {
			dump_err_reg(KRYO_L1_UE, level, errxstatus, errxmisc,
					edev_ctl);
			report_cache_ecc(cpu, DMD_CACHE_LEVEL_L1, DMD_CACHE_TYPE_UE);
		} else {
			dump_err_reg(KRYO_L1_CE, level, errxstatus, errxmisc,
					edev_ctl);
			report_cache_ecc(cpu, DMD_CACHE_LEVEL_L1, DMD_CACHE_TYPE_CE);
		}
		break;
	case L2:
		if (KRYO_ERRXSTATUS_UE(errxstatus)) {
			dump_err_reg(KRYO_L2_UE, level, errxstatus, errxmisc,
					edev_ctl);
			report_cache_ecc(cpu, DMD_CACHE_LEVEL_L2, DMD_CACHE_TYPE_UE);
		} else {
			dump_err_reg(KRYO_L2_CE, level, errxstatus, errxmisc,
					edev_ctl);
			report_cache_ecc(cpu, DMD_CACHE_LEVEL_L2, DMD_CACHE_TYPE_CE);
		}
		break;
	default:
...
static void kryo_check_l3_scu_error(struct edac_device_ctl_info *edev_ctl)
{
	...
	if (KRYO_ERRXSTATUS_VALID(errxstatus) &&
		...
		if (KRYO_ERRXSTATUS_UE(errxstatus)) {
			...
			report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L3, DMD_CACHE_TYPE_UE);
			dump_err_reg(KRYO_L3_UE, L3, errxstatus, errxmisc,
				edev_ctl);
		} else {
			edac_printk(KERN_CRIT, EDAC_CPU, "Detected L3 correctable error\n");
			report_cache_ecc(smp_processor_id(), DMD_CACHE_LEVEL_L3, DMD_CACHE_TYPE_CE);
