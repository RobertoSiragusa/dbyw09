...
static ssize_t lcdkit_watch_aod_alpm(struct platform_device *pdev, const char *buf, size_t count)
{
	...
	case ALPM_HYBRID_CMD:
		...
		LCDKIT_INFO("%s get aod mode 4, switch to mcu\n", __func__);
		mipi_dsi_ulps_cfg(hisifd, 0); // enter ulps
		mdelay(1);
		mipi_to_mcu(hisifd);
		mdelay(1);
	...
	case ALPM_HYBRID_EXIT_CMD:
		LCDKIT_INFO("%s get aod mode 5, switch to AP\n", __func__);
		mipi_dsi_ulps_cfg(hisifd, 0);
		mipi_to_ap(hisifd);
		hisifb_activate_vsync(hisifd);
		mipi_dsi_ulps_cfg(hisifd, 1);
...
void hisifb_get_calicolordata_from_share_mem(struct lcdbrightnesscoloroeminfo *pinfo)
{
	...
	vir_addr = (void *)ioremap_wc(SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_BASE, SUB_RESERVED_BRIGHTNESS_CHROMA_MEM_PHYMEM_SIZE);
	if(vir_addr == NULL) {
...
static int __init lcdkit_probe(struct platform_device* pdev)
{
	...
	if (lcdkit_info.panel_infos.dynamic_gamma_support) {
#if defined(CONFIG_DPU_FB_3660) || defined (CONFIG_DPU_FB_V410)
