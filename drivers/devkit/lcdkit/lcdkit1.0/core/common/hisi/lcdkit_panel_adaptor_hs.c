...
#elif !defined (CONFIG_DPU_FB_V501)
			/* disable de-gamma */
...
#if !defined (CONFIG_DPU_FB_V410) && !defined (CONFIG_DPU_FB_V320) && !defined (CONFIG_DPU_FB_V501) // kirin970 delete bittext function
		/* disable bittext */
...
ssize_t lcdkit_ldo_check_hs_show(char* buf)
{
	...
	for (i = 0; i < g_lcd_ldo_info.lcd_ldo_num; i++)
	{
		...
		{
			cur_val = lpm_adc_get_current(g_lcd_ldo_info.lcd_ldo_channel[i]);
			if(cur_val < 0)
			{
				sum_current = -1;
				break;
