	...
	struct thp_device *tdev, u8 state, u8 touch_click)
{
	...
	if (!state) {
		...
		if (cd->support_vendor_ic_type == GT9896)
			ret = touch_driver_switch_cmd(tdev,
				CMD_TOUCH_GESTURE_SWITCH_GT9896, touch_click);
		else
			ret = touch_driver_switch_cmd(tdev,
	...
	struct thp_udfp_data *udfp_data)
{
	...
	udfp_data->tpud_data.version = (read_buff[0] << MOVE_24BIT) +
		...
		read_buff[3];
	/* read_buff [4:7] = udfp_event */
	udfp_data->tpud_data.udfp_event = (read_buff[4] << MOVE_24BIT) +
	...
	udfp_data->key_event = (read_buff[60] << MOVE_24BIT) +
		...
		read_buff[63];
	thp_log_info("%s:version:%x, udfp_event:%d, aod:%u, key:%u x:%d, y=%d\n",
		__func__, udfp_data->tpud_data.version,
		udfp_data->tpud_data.udfp_event, udfp_data->aod_event,
	...
	bool delay_enable)
{
	...
	u8 bt_connect_value;
	u8 bt_connect_buf[BT_CONNECT_STATUS_BUF_LEN] = {
		0, 0, BT_CONNECT_BUF2, BT_CONNECT_BUF3,
		BT_CONNECT_BUF4, BT_CONNECT_BUF5, 0
	};
	u8 bt_disconnect_buf[BT_CONNECT_STATUS_BUF_LEN] = {
