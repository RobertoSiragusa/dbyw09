	...
	struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (rc_data && rc_data->client) {
		if (event == LCDKIT_TS_RESUME_DEVICE && rc_data->is_suspend)
			crown_handle_event(rc_data->client, rotary_crown_resume);
		else if (event == LCDKIT_TS_SUSPEND_DEVICE && !(rc_data->is_suspend))
...
static int rotary_crown_read_res_from_nv(void)
{
	...
	if (ret) {
		rc_err("nve_direct_access_interface read error %d\n", ret);
		return -EINVAL;
	}
