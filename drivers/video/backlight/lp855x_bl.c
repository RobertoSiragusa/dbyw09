...
static int lp855x_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	/* Set backlight EN gpio */
	np = lp->dev->of_node;
	lp->en_gpio = of_get_named_gpio_flags(np, "en-gpio", 0, NULL);
	if (gpio_is_valid(lp->en_gpio)) {
		ret = gpio_request(lp->en_gpio, "en-gpio");
		if (ret) {
			dev_err(lp->dev, "request for en_gpio failed, rc=%d\n", ret);
			return ret;
		}
		ret = gpio_direction_output(lp->en_gpio, 1);
		if (ret) {
			dev_err(lp->dev, "set en_gpio direction failed, rc=%d\n", ret);
			return ret;
		}
		gpio_set_value(lp->en_gpio, 1);
	} else {
		dev_err(lp->dev, "gpio en is invalid\n");
	}
	/* Set backlight EN gpio */
	if (lp->enable) {
