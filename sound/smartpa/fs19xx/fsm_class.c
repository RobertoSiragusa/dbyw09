                ...
                struct class_attribute *attr, char *buf)
{
    ...
    for (dev = 0; dev < cmd_re25.ndev; dev++) {
        if ((cmd_re25.cal_data[dev].channel & 0x3) == 0) {
            // left/mono
            re25[0] = cmd_re25.cal_data[dev].re25;
        } else {
            // right
            re25[1] = cmd_re25.cal_data[dev].re25;
        }
    }
    ...
    if (pfile->hdr.ndev > 1) {
        // left, right
        size = scnprintf(buf, PAGE_SIZE, "%d,%d", re25[0], re25[1]);
    } else {
        // mono
        size = scnprintf(buf, PAGE_SIZE, "%d", re25[0]);
    }
                ...
                struct class_attribute *attr, char *buf)
{
    ...
    if (pfile->hdr.ndev > 1) {
        ...
        // mono
        size = scnprintf(buf, PAGE_SIZE, "[00,%d,0]",
            payload[3] / 256);
    }
