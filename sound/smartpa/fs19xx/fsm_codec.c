...
int fsm_count_get(struct snd_kcontrol *kcontrol,
            struct snd_ctl_elem_value *ucontrol)
{
    fsm_config_t *cfg = fsm_get_config();
    ucontrol->value.integer.value[0] = cfg->dev_count;
    pr_info("verity status: %ld", ucontrol->value.integer.value[0]);

    return 0;
}

int fsm_count_put(struct snd_kcontrol *kcontrol,
            struct snd_ctl_elem_value *ucontrol)
{
    int verity = ucontrol->value.integer.value[0];

    pr_info("verity status: %d", verity);
...
static const struct snd_kcontrol_new fsm_snd_controls[] =
{
    ...
    SOC_SINGLE_EXT("FSM_Count", SND_SOC_NOPM, 0, 4, 0,
            fsm_count_get, fsm_count_put),
};
