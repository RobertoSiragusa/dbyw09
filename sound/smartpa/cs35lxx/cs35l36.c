				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (asp_width > CS35L36_ASP_WIDTH_16) {
		//for I2S stereo PA, 24 bits data for RX
		...
		//for I2S stereo PA, if 24 bits slot width, then 12 bits for each PA TX Vmon/Imon data
		//if 32 bits slot width, then 16 bits for each PA TX Vmon/Imon data
...
static int cs35l36_component_probe(struct snd_soc_component *component)
{
	...
	if (!cs35l36->pdata.multi_amp_mode) {
		...
		// for stereo spk case
		if (cs35l36->pdata.multi_amp_count == 2) {
			snd_soc_dapm_ignore_suspend(dapm, "L AMP Playback");
			snd_soc_dapm_ignore_suspend(dapm, "L AMP Capture");
			snd_soc_dapm_ignore_suspend(dapm, "L SDIN");
			snd_soc_dapm_ignore_suspend(dapm, "L SDOUT");
			snd_soc_dapm_ignore_suspend(dapm, "L SPK");
			snd_soc_dapm_ignore_suspend(dapm, "L ISENSE");
			snd_soc_dapm_ignore_suspend(dapm, "L VSENSE");
