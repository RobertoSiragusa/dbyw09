...
const char *smartpa_name[SMARTPA_TYPE_MAX] = { "none", "cs35lxx", "aw882xx", "tfa98xx", "fs16xx" };
...
static void set_product_identifier(struct device_node *np, char *buf, int len)
{
	...
	if (of_property_read_bool(np, "need_identify_pa_vendor")) {
		...
		case SIMPLE_PA_AWINIC:
			if (snprintf_s(buf + strlen(buf), len - strlen(buf),
				len - strlen(buf) - 1, "_%s", "aw") < 0)
				pr_err("%s: snprintf_s is failed\n", __func__);
			break;
		case SIMPLE_PA_TI:
			if (snprintf_s(buf + strlen(buf), len - strlen(buf),
				len - strlen(buf) - 1, "_%s", "ti") < 0)
				pr_err("%s: snprintf_s is failed\n", __func__);
			break;
		default:
...
static void product_identifier_init(struct device_node *np, char *buf, int len)
{
	...
	if (of_property_read_string(np, "product_identifier", &string) != 0) {
		if (strncpy_s(buf, len, "default", strlen("default")) != EOK)
			pr_err("%s: strncpy_s is failed\n", __func__);
	} else {
		if (strncpy_s(buf, len, string, strlen(string)) != EOK)
			pr_err("%s: strncpy_s is failed\n", __func__);
	}
...
static void smartpa_type_init(struct device_node *np, char *buf, int len)
{
	...
	if (of_property_read_string(np, "smartpa_type", &string) != 0) {
		if (strncpy_s(buf, len, "none", strlen("none")) != EOK)
			pr_err("%s: strncpy_s is failed\n", __func__);
	} else {
		if (strncpy_s(buf, len, string, strlen(string)) != EOK)
			pr_err("%s: strncpy_s is failed\n", __func__);
...
void hw_set_smartpa_type(const char *buf, int len)
{
	...
	if (memset_s(priv->smartpa_type_str, sizeof(priv->smartpa_type_str),
		0, sizeof(priv->smartpa_type_str)) != EOK)
		pr_err("%s: memset_s is failed\n", __func__);
	if (memcpy_s(priv->smartpa_type_str, sizeof(priv->smartpa_type_str),
		buf, len) != EOK)
...
static void smartpa_num_init(struct device_node *np, char *buf, int len)
{
	...
	if (of_property_read_string(np, "smartpa_num", &string) != 0) {
		if (strncpy_s(buf, len, none_smartpa, strlen(none_smartpa)) != 0) {
			pr_err("%s: strncpy_s error", __func__);
		}
	} else {
		if (strncpy_s(buf, len, string, strlen(string)) != 0) {
			pr_err("%s: strncpy_s error", __func__);
		}
	}
