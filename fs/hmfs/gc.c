...
static void hmfs_datamove_err_handle(struct f2fs_sb_info *sbi,
		...
		void *private_data)
{
	...
	if (dm_info->next_blkaddr && !force_flush && !rescue &&
		...
		HMFS_DM_PU_SIZE)) {
		hmfs_msg(sbi->sb, KERN_ERR,
			"not continuous blkaddr returned by storage, "
			"fs expect next:%u, returned next:%u",
			dm_info->next_blkaddr + HMFS_DM_PU_SIZE,
	...
	sbi->next_pu_size = verify_info.pu_size;
					...
					bool force_flush)
{
	 ...
	 * don't split address.
	 */
	if (!force_flush && dm_info->dm_len > HMFS_DM_PU_SIZE)
		dm_info->dm_len = HMFS_DM_PU_SIZE;

	__hmfs_datamove(sbi, dm_stream_id, force_flush, 0);
	dm_info->dm_len = length - dm_info->dm_len;
	if (dm_info->dm_len)
		dm_info->dm_first_blkaddr += HMFS_DM_PU_SIZE;
	if (force_flush)
		dm_info->cached_last_blkaddr = 0;
}
...
submit:
		...
		if (dm_info->dm_len >= HMFS_DM_PU_SIZE) {
